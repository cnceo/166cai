...
//	 * @return	string
//	 */
//	function compile_binds_old($sql, $binds)
//	{
...
//	}
     ...
     * 作    者：刁寿钧
     * 功    能：修改SQL绑定参数的函数
     *           原来的函数见上面注释掉的compile_binds_old($sql, $binds)，有两个问题：
     *           1.用?分割sql语句，当字符串中包含?时会报错，例如：
     *             SELECT * FROM cp_app_version WHERE content LIKE '%?%'
     *           2.不支持IN的写法
     *           现在这个版本，修复了这两个问题。可以像这样使用IN：
     *             $sql = "SELECT * FROM cp_app_version WHERE id = ? AND delete_flag IN ?";
     *             $testAry = $this->slave->query($sql, array(2, array(0, 1)))->getAll();
     *           补充一句，对于问题1，这里做的修复是遇到引号中的?时不让程序把它当成参数，当然最好一开始就写成
     *             SELECT * FROM cp_app_version WHERE content LIKE ?
     *           然后将'%?%'传给sql语句
     * 修改日期：2015-06-28
     */
...
//	}
     ...
     * 参    数：str
     * 作    者：刁寿钧
     * 功    能：修改对传值进行转义的函数，之前的函数见上面的escape_old($str)，改动说明如下：
     *           1.现在支持IN的写法，传一个数组进来时，要对数组中的元素挨个转义
     *           2.如果传进来的是一个对象，将对象转为字符串进行转义
     *           3.写法形式上改成尽早返回，看着更清楚点
     * 修改日期：2015-06-28
     */
	function escape($str)
	{
