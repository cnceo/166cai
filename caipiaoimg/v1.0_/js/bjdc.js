$(function() {
    ...
    var GGTypes = cx.GGTypes = (function() {
        ...
        me.refreshGGType = function(count) {
            ...
            for (var i = 0; i < matches.length; ++i) {
                ...
                if (matches[i].playOptionsCount.cbf > 0) {
            ...
            if (matches.length == 1 && types.length == 0) {
                $ggTypes.slice(0,1).css('visibility', 'visible').removeClass('selected');
        ...
        toHtml: function(multi, type) {
            ...
            for ( ; ai < associates.length; ++ai) {
                ...
                liTpl += '<tr><td width="180">' + casts.join('*') + '</td>' +
                    '<td width="50">' + multi + '倍</td>' +
                    '<td width="70">' + betMoney + '元</td>' +
    ...
    var CastData = cx.CastData = (function() {
        ...
        function renderDetail() {
            ...
            var minOption = Math.min(len, Math.min(MatchCollection.count(), 1));
            var leftListTpl = '';
            var lenOpt;
            var maxOdd;
            var minOdd;
        ...
        function getCastStr() {
            ...
            castStr += matchStrs.join(',') + '|' + ggTypes;
            return castStr;
        }
    ...
    $('.matches').on('click', '.jqs-option', function() {
        var $this = $(this);
        var $match = $this.closest('tr').prevUntil('.match').last().prev();
