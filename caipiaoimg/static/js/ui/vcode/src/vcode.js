define(['zepto', 'ui/tips/src/tips'],function($, Tips){
    var codes = {
        0: "登录信息有误，请重新输入！"
    };
    ...
    var rules = {
    	username_phonenum: {
	        ...
	        tips: {
	            empty_tip: '请填写手机号或2345账号',
	            format_tip: '请填写正确的手机号或2345账号'
	        }
	    },
	    username: {
			cbfun: validUsername,
	        ...
	        tips: {
	            empty_tip: '请填写2345账号',
	            format_tip: '2-24个字符(支持汉字、字母、数字和邮箱地址)',
	            anti_tip: '2345账号不能为手机号码格式',
	            '1': '此帐号已被注册，请登录或重新输入',
	            '2': '这个账号包含敏感词，请换一个吧'
	        }
	    },
	    ...
	    password: {
	        ...
	        tips: {
	            empty_tip: '请输入密码',
	            format_tip: '支持字母，数字，符号组合，6-16位字符',
				min_len_tip: '最少6个字符',
				max_len_tip: '最多16个字符',
				'1': '该密码和登录密码相同'
	        }
    ...
    function Vform(selector, options) {
        options || (options = {});
        
        this.$form = $(selector);
        // this.$tip = this.$form.find('.tip');
        this.$submit = this.$form.find('.btn-block-confirm');
        this.$eles = this.$form.find('input');
    ...
    $.fn.extend({
    VformA:function(selector, options){
    	options || (options = {});
        ...
        validate: function(ele) {
            ...
            if (ruleName in rules) {
                ...
                if (rule.anti_pattern) {
                    if (rule.anti_pattern.test(val)) {
                        this.renderTip(rule.tips.anti_tip);
                        return false;
                    }
                }
                if ('min' in rule) {
	...
	function validPassword( pass, renderTip ){
		...
		if( pass == self.$form.find('input[name="username"]').val() ){
			renderStr = '<div>密码不能与2345帐号一致，请重新输入</div>';
