(function($) {
                ...
                $(options).each(function(ks, vs) {
                    $(vs.split('/')).each(function(k, v) {
                        ...
                        if (matches) {
                            ...
                            if(playType == 'DXF') {
                            	cast[matchId].preScore = matches[4];
                            }
    ...
    function renderMatch(matchId, cast, matchInfo) {
        ...
        $(playTypes).each(function(k, playType) {
            if (playType in cast) {
                ...
                tpl += '<td>' + _renderOptions(playType, cast[playType], cast.let, matchInfo, cast.preScore) + '</td>'; // 投注方案
                tpl += '</tr>';
                playCount += 1;
            }
        });
    ...
    function _renderOptions(playType, options, let, matchInfo, preScore) {
        var tpl = '';
        var isRight = false;
        var castName = '';
        var casts = [];
