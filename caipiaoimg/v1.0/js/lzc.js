$(function() {
    ...
    Match.prototype.addOption = function(playType, option, odd) {
        this.playOptions[playType][option] = odd;
    ...
    var CastData = cx.CastData = (function() {
        ...
        me.renderMinMaxMoney = function() {
            ...
            if (matchesCount < fixedGGOptions[0] || ggCount < 1) {
                $minMoney.html('0.00');
                $maxMoney.html('0.00');
                return ;
            }
        ...
        $('.cast-panel').find('.submit').click(function(e) {
            ...
            if (MatchCollection.count() < fixedGGOptions[0]) {
                new cx.Alert({
                    content: '请至少选择' + fixedGGOptions[0] + '场比赛'
                });
                return ;
            }
            var url = cx.url.getBusiUrl('ticket/order/create');
        ...
        function getCastStr() {
            ...
            for (var i = 0; i < matches.length; ++i) {
                ...
                castStr[match.options.mid - 1] = castOptions;
            }
            castStr = castStr.join(',') + ':1:1@' + endTime + '@' + mids.join(' ');
            return castStr;
        }
    ...
    function onClick($el, $match, type) {
        ...
        //alert("mid="+mid+"-let="+let+"-val="+val+"-odd="+odd+"-wid="+wid+"-home="+home+"-away="+away+"-$el="+$el);
       if ($el.hasClass(selectedClass)) {
            ...
            if (cx.MatchCollection.get(mid).playOptionsCount[type] == 0) {
                $match.find('.open-' + type).removeClass('selected').html('选择');
            }
