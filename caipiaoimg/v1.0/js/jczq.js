$(function() {
        ...
        $ggTypes.click(function() {
            ...
            if ($this.hasClass('selected')) {
                ...
                if ($this.hasClass('single')) {
                    var matches = MatchCollection.getArray();
                    var playTypes = ['spf', 'rqspf', 'jqs', 'cbf', 'bqc'];
                    for (var i = 0; i < matches.length; ++i) {
                        var match = matches[i];
    ...
    var CastData = cx.CastData = (function() {
        ...
        function renderPrizeList(len, oAllCasts) {
            ...
            for (var i = 0; i < maxCombs.length; ++i) {
                ...
                for (var j = 0; j < tmp.length; ++j) {
                    if (tmp[j].length > 1) {
                        ...
                        key = tmp[j][0].options.mid + '_' + tmp[j][0].playType + '_' + tmp[j][0].option;
                    }
            ...
            var rMinSum = Math.min(minSum, maxSum);
            var rMaxSum = Math.max(minSum, maxSum);
            return '<tr class="detail-li"><th width="25%">中' + len + '场</th>' +
                   '<td width="75%"><h4>最大奖金：' + round(rMaxSum) + '</h4><h5>最小奖金：' + round(rMinSum) + '</h5></td></tr>';
        }
        ...
        function getCastStr() {
            ...
            if (singleIndex > -1) {
                if (ggTypes.length == 1) {
                    castStr += matchStrs.join(',') + '|1*1';
                } else {
                    ggTypes.splice(singleIndex, 1);
                    castStr += matchStrs.join(',') + '|' + ggTypes.join(',') + ';HH|' + matchStrs.join(',') + '|1*1';
                }
            } else {
                castStr += matchStrs.join(',') + '|' + ggTypes.join(',');
            }
            castStr += '@' + endTime + '@' + mids.join(' ');
            return castStr;
        }
    ...
    function onClick($el, $match, type) {
        ...
        var spfFu = $match.data('spf_fu');
        var rqspfFu = $match.data('rqspf_fu');
        var cbfFu = $match.data('cbf_fu');
        var jqsFu = $match.data('jqs_fu');
        var bqcFu = $match.data('bqc_fu');
    ...
    var SelectedMatches = (function() {
        ...
        function renderMatch(i, match) {
            var vs = match.options.home + ' <em>VS</em> ' + match.options.away;
            var optionTpl = '';
            for (var playType in match.playOptions) {
            ...
            var tpl = '<tr data-mid="' + match.options.mid + '"><td width="180" style="text-align:center">' + vs + '</td><td width="182">' + optionTpl + '</td><td width="63" style="text-align:center"><a class="del-match" data-index="' + i + '">删除</a></td></tr>';
