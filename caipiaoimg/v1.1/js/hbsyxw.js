...
$(function() {
	...
	for (var j = 2; j <= 8; j++){
		boxes['rx'+j] =  new cx.BoxCollection('.rx'+j+' .default .box-collection',{lotteryId: cx.Lottery.HBSYXW});
	    for (var i = 1; i <= 1; ++i) {
	        boxes['rx'+j].add(new cx.BallBox('.rx'+j+' .default .ball-box-'+i, {
	        	lotteryId: cx.Lottery.HBSYXW,
	            amount: 11,
	            min: j,
	            minBall: 1,
	            mutex: 1,
	    ...
	    if (j != 8) {
	    	boxes['rx'+j+'dt'] =  new cx.BoxCollection('.rx'+j+' .dt .box-collection',{lotteryId: cx.Lottery.HBSYXW});
		    for (i = 1; i <= 1; ++i) {
    ...
    boxes['q1'] =  new cx.BoxCollection('.q1 .default .box-collection',{lotteryId: cx.Lottery.HBSYXW});
    for (var i = 1; i <= 1; ++i) {
    ...
    for (j = 2; j <=3; j++) {
    	boxes['qzu'+j] =  new cx.BoxCollection('.qzu'+j+' .default .box-collection',{lotteryId: cx.Lottery.HBSYXW});
        for (var i = 1; i <= 1; ++i) {
        ...
        boxes['qzu'+j+'dt'] =  new cx.BoxCollection('.qzu'+j+' .dt .box-collection',{lotteryId: cx.Lottery.HBSYXW});
        for (i = 1; i <= 1; ++i) {
	    ...
	    boxes['qzhi'+j] =  new cx.BoxCollection('.qzhi'+j+' .default .box-collection',{lotteryId: cx.Lottery.HBSYXW});
	    for (var i = 1; i <= j; ++i) {
