(function($) {
    ...
    function renderMatch(matchId, cast, matchInfo, ticketInfo) {
        ...
        var score = '-';
        if(matchInfo.score){
        	score = matchInfo.score + '<br/>(' + matchInfo.halfScore + ')';
        }
        if(matchInfo.mStatus == 1)
        {
        	score = '场次取消';
        }
        tpl += '<td rowspan="' + rowspan + '">' + getMatchId(matchId).join(' ') + '</td>';
        tpl += '<td rowspan="' + rowspan + '">' + _formatDateTime(matchInfo.dt) +  '</td>';
		tpl += '<td rowspan="' + rowspan + '"><div><span class="tar" style="float:left; width: 50%">' + matchInfo.home + '&nbsp;&nbsp;</span><span class="tal" style="float:left; width: 50%">&nbsp;&nbsp;' +  matchInfo.away +  '</span></div></td>';
		tpl += '<td rowspan="' + rowspan + '"><p>' + score + '</p></td>';	// 比分
        var playCount = 0;
        $(playTypes).each(function(k, playType) {
            if (playType in cast) {
                ...
                var playName = playNames[playType];
                if(playType == 'RQSPF'){
                	playName = playName + '(' + cast.let + ')';
                }
                tpl += '<td>' + playName + '</td>';	// 玩法
                tpl += '<td>' + _renderAward(playType, cast.let, matchInfo) + '</td>';	// 彩果
                // 获取实际开奖赔率信息
                if(ticketInfo !== null && ticketInfo !== undefined){
                    ticketData = ticketInfo[matchId][playType];
                }
                tpl += '<td>' + _renderOptions(playType, cast[playType], cast.let, matchInfo, ticketData) + '</td>'; // 投注方案
                tpl += '<td>' + _renderMoney(cast[playType]) + '</td>'; // 投注金额
                tpl += '</tr>';
                playCount += 1;
            }
        });
	...
	function _renderAward(playType, let, matchInfo) {
        var tpl = '<p>';
        var castName = '-';
        if(matchInfo.score){
        	var result = _getCastOption(playType, matchInfo.score, matchInfo.halfScore, let);
        	castName = _getCastName(playType, result);
        }
        tpl += castName + '</p>';
        return tpl;
	}
    ...
    function _renderMoney(options) {
    	var tpl = '<div class="jc-inTable-pd">';
    	var money = 0;
    	$(options).each(function(k, option) {
    		money = parseFloat(option.multi * 2);
    		tpl += '<p>' + formatNum(money) +'元</p>';
    	});
    	tpl += '</div>';
    	return tpl;
    }
