(function() {
    ...
    var Lottery = cx.Lottery = (function() {
        ...
        me.getRule = function(lotteryId, playType) {
            if (lotteryId == Lottery.DLT) {
                return '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">５</span>个红球和<span class="num-blue">２</span>个蓝球';
            } else if (lotteryId == Lottery.SSQ) {
                return '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">６</span>个红球和<span class="num-blue">１</span>个蓝球';
    ...
    BoxCollection.prototype = {
        ...
        rand1: function(lotteryId) {
        	var randStrs = {
                balls: [],
                betNum: 0
            };
        	var arr = cx.Lottery.getMinLength(lotteryId);
        	var amount = cx.Lottery.getAmount(lotteryId);
        	for (i in arr)
        	{
        		randStrs.balls[i] = [];
...
//            });
            this.$el.find('.clear-balls').click(function() {
            	clickCount('clear-balls');
                self.removeAll();
            });
            this.$el.find('.filter-bigs').click(function() {
            	clickCount('rand-select');
                self.removeAll();
                var ball;
    ...
    var CastBasket = cx.CastBasket = function(selector, options) {
        ...
        this.$betNum = this.$el.find('.bet-area-txt').find('.num-multiple');
        this.$betMoney = this.$el.find('.bet-area-txt').find('.num-money');
        this.betNum = 0;
        this.betMoney = 0;
        this._betMoney = 2;
    ...
    CastBasket.prototype = {
        init: function() {
            ...
            this.$el.find('.add-basket').click(function() {
            	clickCount('add-basket');
            	if (self.boxes.calcMoney() > 20000) {
            		cx.Alert({content: "<i class='icon-font'>&#xe611;</i>单笔订单金额最高<span class='num-red'>２万</span>元"});
            	}else if (!self.boxes.isValid()) {
            		cx.Alert({
                	...
                	{
                		self.edit(balls, $(this).attr('edit'));
                	}
                    self.boxes.removeAll();
                    $('html, body').animate({scrollTop: $('.btn-betting').offset().top + $('.btn-betting')[0].scrollHeight - $(window).height()});
            	}
            });
            this.$el.find('.rand-cast').click(function() {
                var $this = $(this);
                var amount = parseInt($this.data('amount'), 10);
                clickCount('rand-cast-'+amount);
                var rand;
                for (var i = 0; i < amount; ++i) {
                    rand = self.boxes.rand1(self.lotteryId);
                    self.add(rand);
                }
            });
            this.$el.on('click', '.remove-str', function() {
            	clickCount('remove-str');
                var $li = $(this).closest('li');
                if($li.attr('data-index') === $(".add-basket").attr('edit'))
                {
                	self.clearButton();
                }
                $li.remove();
            ...
            this.$el.on('click', '.modify-str', function() {
            	clickCount('modify-str');
            	var startindex = cx.Lottery.getStartIndex(self.lotteryId);
            	for (i in self.boxes.boxes) {
            		var preballs = $(this).parent().find('span').eq(parseInt(i)+1).html();
  	              	self.boxes.boxes[i].removeAll();
  	              	$(preballs.split(' ')).each(function(k, ball){
  	              		ball = parseInt(ball, 10);
  	              		if(!isNaN(ball))
  	              		{
  	              			self.boxes.boxes[i].addBall(ball + '') ;
  	              			self.boxes.boxes[i].$balls.eq(ball - parseInt(startindex[i])).addClass('selected'); 
  	              		}
  	              	})
            	}
	            self.boxes.renderBet();
	            $(".add-basket").attr('edit', $(this).parent().attr('data-index'));
            ...
            this.$el.find('.clear-list').click(function() {
            	clickCount('clear-list');
                self.removeAll();
                self.clearButton();
            });

            this.$el.find('.submit').click(function(e) {
            	clickCount('buy-submit');
                var $this = $(this);
                ...
                if ($.inArray(data.lid,maxMoneyLid_2w) >= 0){
                    if ( data.money >20000 ) {
                        new cx.Alert({content: "<i class='icon-font'>&#xe611;</i>订单金额需小于<span class='num-red'>２万</span>元，请修改订单后重新投注"});
                        return ;
                    }
                }else{
                    if ( data.money >200000 ) {
                        new cx.Alert({content: "订单金额需小于20万元，请修改订单后重新投注"});
                        return ;
                    }
                }
        ...
        edit: function(balls, id) {
            ...
            this.$castList.find("li[data-index="+id+"]").replaceWith(this.renderString(balls.balls, id, this._betMoney * balls.betNum, true));
            this.betNum += balls.betNum-betNum;
            this.betMoney += this._betMoney * this.betNum-this._betMoney * betNum;
            this.renderAllBet();
            this.clearButton();
        },
        ...
        clearButton: function(){
        	$(".add-basket").removeAttr('edit');
    		$(".add-basket").html('添加到投注区<i class="icon-font">&#xe614;</i>');
        },
        renderString: function(allBalls, index, money, hover) {
        	var minArr = cx.Lottery.getMinLength(this.lotteryId);
        	var tpl = '<li ';
        	if(hover) {
        		tpl += ' class="hover"'
        	}
            ...
            for (var pi = 0; pi < allBalls.length; ++pi) {
                ...
                for (var bi = 0; bi < allBalls[pi].length; ++bi) {
                	if(this.lotteryId == Lottery.DLT || this.lotteryId == Lottery.SSQ || this.lotteryId == Lottery.QLC)
                	{
                		tmpTpl += pad(allBalls[pi][bi]) + ' ';
                	}else {
                		tmpTpl += allBalls[pi][bi] + ' ';
                	}
                }
                // if (pi > 0 && (this.lotteryId == Lottery.DLT || this.lotteryId == Lottery.SSQ)) {
                //     tmpTpl += '</span>';
            ...
            tpl += '</div><a href="javascript:;" class="remove-str">删除</a><a href="javascript:;" class="modify-str">修改</a><span class="bet-money">'+ money +'元</span>';
            tpl += '</li>';
