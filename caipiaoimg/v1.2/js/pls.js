(function() {
	...
	cx.Lottery = (function(){
		...
		var lid = cx.Lid.PLS, me = {};
		
		me.lid = lid;
		me.playTypes = {'default':'1', zx:'1', z3:'2', z6:'3'}, me.jiangjin = {zx:'1040', z3:'346', z6:'173'};
...
$(function() {
	...
	cx.BoxCollection.prototype.renderBet = function() {
    	if(this.$el.find('.num-red').length > 0) this.$el.find('.num-red').html(this.getNum(0));
        ...
        if (rule['status'] === true) {
        	if(this.$el.find('.sub-txt').length > 0) {
        		var playType = this.getType(), money = cx.Lottery.jiangjin[playType];
	...
	cx.BoxCollection.prototype.rand1 = function(playType) {
		...
		startindex = cx.Lottery.getStartIndex(playType), arr = cx.Lottery.getMinLength(playType);
		randStrs.betNum = 1;
	...
	cx.BoxCollection.prototype.z3ballsplit = function(aball){
    	...
    	$.each(ball, function(i, a){
    		$.each(ball, function(j, b){
    			if (i != j) {
    				balls[k] = {balls:[{'tuo' : [a, a, b]}], betNum: 1, betMoney: 2, playType: "z3"};
    				k++;
    			}
    		})
    	})
    ...
    basket = new cx.CastBasket('.cast-basket', {boxes: boxes, chases: chase, chaseLength: chaselength, multi: MULTI, multiModifier: multiModifier, playType: type, tabClass:'current', tab: 'bet-tab-hd li'});
    cx._basket_ = basket;
});