...
(function() {
	...
	cx.Lottery = (function(){
		...
		var lid = cx.Lid.SYXW, me = {}; 
		
		me.lid = lid;
		me.playTypes = {'default': '05', q1: '01', rx2: '02', rx2dt: '02', rx3: '03', rx3dt: '03', rx4: '04', rx4dt: '04', rx5: '05', rx5dt: '05', rx6: '06', rx6dt: '06', rx7: '07', rx7dt: '07', rx8: '08', qzhi2: '09', qzhi3: '10', qzu2: '11', qzu2dt: '11', qzu3: '12', qzu3dt: '12', lexuan3: '13', lexuan4: '14', lexuan5: '15'};
		
		me.getPlayTypeByMidCode = function(midCode, postCode) {
			var data = {'01':'q1', '02':'rx2', '03':'rx3', '04':'rx4', '05':'rx5', '5':'rx5', '06':'rx6', '07':'rx7', '08':'rx8', '09':'qzhi2', '10':'qzhi3', '11':'qzu2', '12':'qzu3', '13':'lexuan3', '14':'lexuan4', '15':'lexuan5'}, playType = (midCode in data) ? data[midCode] : 'rx8';
			return postCode == '05' ? playType+"dt" : playType;
	    }
		
		me.jiangjin = {q1:'13',rx2:'6',rx2dt:'6',rx3:'19',rx3dt:'19',rx4:'78',rx4dt:'78',rx5:'540',rx5dt:'540',rx6:'90', rx6dt:'90',rx7:'26',rx7dt:'26',rx8:'9',qzhi2:'130',qzhi3:'1170',qzu2:'65',qzu2dt:'65',qzu3:'195',qzu3dt:'195'};
		...
		me.getCastPost = function( playType) {
	        var CAST_POST = {'default': '01', 'rx2': '01', 'rx2dt': '05', 'rx3': '01', 'rx3dt': '05', 'rx4': '01', 'rx4dt': '05', 'rx5': '01', 'rx5dt': '05', 'rx6': '01', 'rx6dt': '05', 'rx7': '01', 'rx7dt': '05', 'rx8': '01', 'q1':'01', 'qzhi2': '01', 'qzhi3': '01', 'qzu2': '01', 'qzu2dt': '05', 'qzu3': '01', 'qzu3dt': '05', 'lexuan3': '01', 'lexuan4': '01', 'lexuan5': '01'};
	        return (playType in CAST_POST) ? CAST_POST[playType] : CAST_POST['default'];
	    };
	    ...
	    me.getMinLength = function(playType) {
	    	var data = {'default': [1], 'rx2': [2], 'rx2dt': [2], 'rx3': [3], 'rx3dt': [3], 'rx4': [4], 'rx4dt': [4], 'rx5': [5], 'rx5dt': [5], 'rx6': [6], 'rx6dt': [6], 'rx7': [7], 'rx7dt': [7], 'rx8': [8], 'q1': [1], 'qzu2': [2], 'qzu2dt': [2], 'qzhi2': [1, 1], 'qzu3': [3], 'qzu3dt': [3], 'qzhi3': [1, 1, 1], 'lexuan3': [1, 1, 1], 'lexuan4': [4], 'lexuan5': [5]}
	    	playType = playType || 'default';
	    	return data[playType];
	    }
	    
	    me.getAmount = function(playType) {
	    	var data = {'default': [11], 'rx2': [11], 'rx2dt': [11], 'rx3': [11], 'rx3dt': [11], 'rx4': [11], 'rx4dt': [11], 'rx5': [11], 'rx5dt': [11], 'rx6': [11], 'rx6dt': [11], 'rx7': [11], 'rx7dt': [11], 'rx8': [11], 'q1': [11], 'qzu2': [11], 'qzu2dt': [11], 'qzhi2': [11, 11], 'qzu3': [11], 'qzu3dt': [11], 'qzhi3': [11, 11, 11], 'lexuan3': [11, 11, 11], 'lexuan4': [11], 'lexuan5':[11]}
	    	playType = playType || 'default';
	    	return data[playType];
	    }
	    
	    me.getStartIndex = function(playType) {
	    	var data = {'default': [1], 'qzhi2': [1, 1], 'qzhi3': [1, 1, 1], 'lexuan3': [1, 1, 1]};
	    	...
	    	switch (playType) {
		        ...
		        case 'lexuan3':
		        	for (i in state) {
		        		if (state[i].indexOf('1') > -1) {
		        			result['status'] = false;
	    					result['content'] = '<i class="icon-font">&#xe611;</i>每位选择1个号码，且相互不重复';
	                		break;
		        		}
		        	}
		            break;
		        ...
		        case 'lexuan5':
		        	if (state[0] == '1') {
		        		result['status'] = false;
						result['content'] = '<i class="icon-font">&#xe611;</i>请选择<span class="num-red">'+index+'</span>个号码';
		        	}
		        	break;
		        default:
		        break;
...
$(function() {
		...
		this.$el.find('.add-basket').click(function() {
    		...
    		if (rule['status'] !== true) {
    			...
    			if (self.getType().indexOf('lexuan') > -1){
    				...
    				self.edit = 0;
        			var balls = self.splitlexuanBalls(balls);
	...
	cx.BoxCollection.prototype.rand1 = function(playType) {
    	...
    	var startindex = cx.Lottery.getStartIndex(playType), arr = cx.Lottery.getMinLength(playType), amount = cx.Lottery.getAmount(playType);
    	randStrs.betNum = 1;
    	...
    	for (i in arr) {
    		...
    		while (randStrs.balls[i]['tuo'].length < arr[i]) {
        		...
        		if ($.inArray(playType, ['qzhi2', 'qzhi3', 'lexuan3']) > -1) {
        			var eflag = true;
	...
	cx.BoxCollection.prototype.splitlexuanBalls = function(balls) {
			...
			$.each(balls.balls[0]['tuo'], function(i0, v0){
				$.each(balls.balls[1]['tuo'], function(i1, v1){
					$.each(balls.balls[2]['tuo'], function(i2, v2){
						result[autoId] = {balls : [{'tuo':[v0]},{'tuo':[v1]},{'tuo':[v2]}], betNum : 1, betMoney : 6, playType : balls.playType};
						autoId++;
					})
				})
			})
					...
					for (b = a+1; b < tuo.length; b++) {
						for (c = b+1; c < tuo.length; c++) {
							for (d = c+1; d < tuo.length; d++) {
								result[autoId] = {balls : [{'tuo':[tuo[a], tuo[b], tuo[c], tuo[d]]}], betNum : 1, betMoney : 10, playType : balls.playType};
								autoId++;
							}
						}
					}
						...
						for (c = b+1; c < tuo.length; c++) {
							for (d = c+1; d < tuo.length; d++) {
								for (e = d+1; e < tuo.length; e++) {
									result[autoId] = {balls : [{'tuo':[tuo[a], tuo[b], tuo[c], tuo[d], tuo[e]]}], betNum : 1, betMoney : 14, playType : balls.playType};
									autoId++;
								}
	...
	cx.BallBox.prototype.BallTriger = function ($el, type) {        	
        ...
        if ($el.hasClass('selected')) {
        	...
        	if ($.inArray(this.playType, ['qzhi2', 'qzhi3', 'lexuan3']) > -1) {
	...
	cx.CastBasket.prototype.init = function(){
        ...
        this.$el.on('click', '.modify-str', function() {
        	...
        	if(string.playType !== self.playType) {
        		var flag = 'lexuan';
        		if (string.playType.indexOf('lexuan') === -1) flag = string.playType.replace(/dt/, '');
	...
	cx.CastBasket.prototype.setType = function(type) {
		this.playType = type;
		if (this.playType.indexOf('lexuan') > -1) {
			var t = this.playType.replace(/\d/, '');
			$("."+this.tab+".last").attr('data-type', this.playType);
		}else {
        ...
        if (this.playType.indexOf('lexuan') > -1) {
        	$("."+this.playType+" .pick-area-time").html("<em><b>"+ISSUE.substring(2, 8)+"</b></em>期剩余<span>"+maketstr(tm)+"</span><i class='arrow'></i>");
        }else if (this.playType.indexOf('dt') > -1) {
	...
	for (var j = 2; j <= 8; j++){
	    ...
	    boxes['rx'+j].add(new cx.BallBox('.rx'+j+' .default .ball-box-1', {playType: 'rx'+j}));
	    
	    if (j != 8) {
	    	boxes['rx'+j+'dt'] =  new cx.BoxCollection('.rx'+j+' .dt .box-collection');
		    boxes['rx'+j+'dt'].add(new cx.BallBox('.rx'+j+' .dt .ball-box-1:last', {hasdan: true, dmax: j-1, tmin: 2, dtmin: j+1, playType: 'rx'+j+'dt'},'.rx'+j+' .dt .ball-box-1:first'));
	    }
	}
    
    boxes['q1'] =  new cx.BoxCollection('.q1 .default .box-collection');
    boxes['q1'].add(new cx.BallBox('.q1 .default .ball-box-1', {playType: 'q1'}));
    
    for (j = 2; j <=3; j++) {
    	boxes['qzu'+j] =  new cx.BoxCollection('.qzu'+j+' .default .box-collection');
        boxes['qzu'+j].add(new cx.BallBox('.qzu'+j+' .default .ball-box-1', {playType: 'qzu'+j}));
        
        boxes['qzu'+j+'dt'] =  new cx.BoxCollection('.qzu'+j+' .dt .box-collection');
        boxes['qzu'+j+'dt'].add(new cx.BallBox('.qzu'+j+' .dt .ball-box-1:last', {hasdan: true, dmax: j-1, tmin: 2, dtmin: j+1, playType: 'qzu'+j+'dt'},'.qzu'+j+' .dt .ball-box-1:first'));
    ...
    boxes['lexuan3'] =  new cx.BoxCollection('.lexuan .lexuan3 .box-collection',{betMoney:6});
    for (var i = 1; i <= 3; ++i) {
        boxes['lexuan3'].add(new cx.BallBox('.lexuan .lexuan3 .ball-box-'+i, {playType: 'lexuan3'}));
    }
    boxes['lexuan4'] = new cx.BoxCollection('.lexuan .lexuan4 .box-collection',{betMoney:10});
    boxes['lexuan4'].add(new cx.BallBox('.lexuan .lexuan4 .ball-box-1', {playType: 'lexuan4'}));
    boxes['lexuan5'] = new cx.BoxCollection('.lexuan .lexuan5 .box-collection',{betMoney:14});
    boxes['lexuan5'].add(new cx.BallBox('.lexuan .lexuan5 .ball-box-1', {playType: 'lexuan5'}));
