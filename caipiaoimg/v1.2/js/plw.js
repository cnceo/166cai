(function() {
	...
	cx.Lottery = (function(){
		...
		var lid = cx.Lid.PLW, me = {};
		
		me.lid = lid;
		
		me.playTypes = {'default':'1'}, jiangjin = {'default':'100000'};
	    ...
	    me.getRule = function(playType, state) {
	    	if (state[0].indexOf('1') > -1)  return {'status':false, 'content':'<i class="icon-font">&#xe611;</i>每位至少选择１个号码', 'size':'18'}
...
$(function() {
    ...
    for(var i = 1; i <= 5; ++i){
        boxes['default'].add(new cx.BallBox('.ball-box-' + i, {playType: 'default'}));
    }
 
    var multiModifier = new cx.AdderSubtractor('.multi-modifier'),
    basket = new cx.CastBasket('.cast-basket', {boxes: boxes, chases: chase, chaseLength: chaselength, multi: MULTI, multiModifier: multiModifier, tabClass:'current',  tab: 'bet-tab-hd li'});
    cx._basket_ = basket;
});
