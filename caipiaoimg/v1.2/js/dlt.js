(function() {
	...
	cx.Lottery = (function(){
		...
		var lid = cx.Lid.DLT, me = {}; 
		
		me.lid = lid;
		me.playTypes = {'default': 1, 'zj' : 2, dt : 135};
		
		me.getPlayTypeByMidCode = function(midCode) {
			var data = {'1':'default', '2':'default', '135':'dt'};
		...
		me.getCastPost = function( playType) {
	        var CAST_POST = {'default': '1', 'zj' : '1', dt:'5',}
	    	...
	    	switch (playType) {
	            case 'dt':
            		for (i in state){
            			switch (state[i]) {
	        				case '11':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">1</span>个前区胆码';
	        					break;
	        				case '40':
	        					if ($.inArray('10', state) > -1) {
	        						...
	        						result['status'] = false;
			    					result['content'] = '<i class="icon-font">&#xe611;</i>前区胆码＋前区拖码≥<span class="num-red">6</span>个';
	        					}
	        					break;
            				case '21':
            				case '22':
            				case '23':
            					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">2</span>个前区拖码';
	        					break;
            				case '31':
            				case '32':
            				case '33':
	        				...
	        				case '50':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>最多选择<span class="num-red">4</span>个前区胆码';
	        					break;
	        				case '02':
	        				case '12':
	        				case '42':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">2</span>个后区拖码';
	        					break;
	        				case '05':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>最多选择<span class="num-red">1</span>个后区胆码';
	        					break;
	        			}
            		}
            		break;
        		case 'ddsh':
        			for (i in state){
        				switch (state[i]) {
		        			case '10':
		        				result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>前区最多可定<span class="num-red">4</span>个胆码<span class="pop-small">(胆码超出后自动做杀号处理)</span>';
	        					break;
	        				case '20':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>前区最多可杀<span class="num-red">30</span>个号码';
	        					break;
	        				case '02':
	        					result['status'] = false;
		    					result['content'] = '<i class="icon-font">&#xe611;</i>后区最多可杀<span class="num-red">10</span>个号码';
	        					break;
	        			}
        			}
        			break;
        		...
        		default:
        			if ($.inArray('00', state) === -1) {
        				result['status'] = false;
    					result['content'] = '<i class="icon-font">&#xe611;</i>请至少选择<span class="num-red">５</span>个前区和<span class="num-blue">２</span>个后区';
        			}
        			break;
            }
		    return result;
...
$(function() {
	...
	cx.BallBox.prototype.ShaTriger = function($el) {
        ...
        if (this.index == 0 || this.dans.length > 0) {
	...
	cx.CastBasket.prototype.init = function(){
        ...
        this.$el.on('click', '.zjtz,.add-bets', function(e) {
        	if (e.target.id === 'addBets' || e.target.id === 'zjtz') {
        		var betMoney = 2;
        		self.zj = false;
	...
	cx.CastBasket.prototype.setType = function(type) {
		this.playType = type;
	    if(type=='dssc') window.location.href = baseUrl+'ssq/dssc';
		if (type === 'ddsh') {
			$(".ptips-bd-b").hide();
		}else {
			$(".p1tips-bd-b").show();
	...
	cx.CastBasket.prototype.renderString = function(allBalls, index, hover, noedit) {
        ...
        var ballTpl = [], dt = false, danTpl = '', self=this;
	...
	$.extend(cx.castData, {'toCastString':function(subStrings, zj){
		...
		$.each(subStrings, function(k, subString) {
			var playType = subString.playType || 'default', type = playType;
        	if (zj) type = (playType == 'default') ? 'zj' : playType;
        	var midStr = ':' + cx.Lottery.playTypes[type];
        	if (zj) type = (playType == 'dt') ? 'zj' : playType;
            var postStr = ':' + cx.Lottery.getCastPost(type);
            preArr = [];
            $.each(subStrings[k].balls, function(j, ball) {
	...
	boxes = {'default' : new cx.BoxCollection('.default .box-collection'), 'dt' : new cx.BoxCollection('.dt .box-collection'), 'ddsh' : new cx.BoxCollection('.ddsh .box-collection')};
    ...
    boxes['dt'].add(new cx.BallBox('.dt .post-box:last', {
        dmax: 1,
        tmin: 2,
        dmax: 1,
        dtmin: 1,
