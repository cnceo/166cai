...
<!--容器-->
<div class="wrap cp-box bet-jc jczq-spf">
                    ...
                    <dd class="select-opt">
                        <div class="select-opt-in">
                            <?php foreach ($dates as $key => $value): ?>
                                <a href="<?php echo $baseUrl; ?>jczq/spf?date=<?php echo $value; ?>"><?php echo $key; ?></a>
                            <?php endforeach; ?>
                        </div>
                    </dd>
                </dl>
                                            ...
                                            <tr
                                                ...
                                                <?php foreach (array(3, 1, 0) as $r): ?>
                                                    <td class="jc-option  <?php if($match['full_score']) { echo "hasmatched";}?> <?php echo $r ? '' : 'no-bd-r' ?>">
                                                        <a class="spf-option  <?php if($match['result']['spf']==$r && $match['full_score'] && $match['m_status']!=1){ echo "bingo";} ?>"
                                                           onselectstart="return false;"
                                                           style="-moz-user-select: none"
                                                           data-val="<?php echo $r; ?>"
                                                           data-odd="<?php if($match['m_status']==1){echo 1;}
                                                           elseif($match['result']['spf']==$r || !$match['full_score'])
                                                           {echo $match['spfSp' . $r];}else{ echo 0;} ?>">
                                                            <b><?php echo $match['spfSp' . $r]; ?></b>
                                                        </a>
                                                    </td>
                                                <?php endforeach; ?>
                                                <td class="oyx">
         ...
         $("#xstsbs").click(function(){
            if($(this).is(':checked')){
              if($(".oldmatch").hasClass("hidden")){
                ...
                $.get('/jczq/getOldMatch?type=spf',function(data){
                    $(".oldmatch").html(data);
                });
              }
            }else{
