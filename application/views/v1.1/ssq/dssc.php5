...
var SSQ_ISSUE = "<?php echo $info['next']['issue']?>", ENDTIME = "<?php echo date('Y-m-d H:i:s', $info['next']['seFsendtime']/1000)?>", tzjqurl = "<?php echo $tzjqurl?>", chases = $.parseJSON('<?php echo json_encode($chases)?>'), 
...
<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.1/js/ssq.js');?>"></script>
        ...
        <div class="bet-main">
        	<div class="bet-link-bd">
        		<div class="bet-pick-area bet-tab">
	        		<div class="bet-tab-hd _bet_tab_hd"><ul><li><a href="/ssq?playType=default">普通投注</a></li><li><a href="/ssq?playType=dt">胆拖投注</a></li><li data-type="ddsh"><a href="/ssq?playType=ddsh">定胆杀号</a></li><li  class="current dssc"><a href="/ssq/dssc">单式上传</a></li></ul></div>
	            <!-- 数字彩投注区 start -->
	                        ...
	                        <div class="chase-number-table">
	                            <table class="chase-number-table-hd">
	                                <thead>
	                                    ...
	                                    <tr><th>序号</th><th class="tal"><input checked type="checkbox">追<input type="text" value="10" data-max="<?php echo count($chases) > 50 ? 50 : count($chases)?>" class="ipt-txt follow-issue _follow-issue">期<s>（共50期）</s></th>
	                                    <th><input type="text" class="ipt-txt follow-multi _follow-multi" data-max="99" value="<?php echo $multi?>">倍</th><th>方案金额（元）</th><th>预计开奖时间</th></tr>
	                                </thead>
	                            </table>
