...
<head>
    ...
    <script>
    var KLPK_ISSUE = "<?php echo $info['cIssue']['seExpect']?>", ISSUE = KLPK_ISSUE, tm = <?php echo $info['cIssue']['seFsendtime']/1000-time();?>,
    ...
    <script src="<?php echo getStaticFile('/caipiaoimg/v1.1/js/klpk.min.js');?>" type="text/javascript"></script>
...
<body>
    ...
    <div class="bet-klpk">
        <div class="bet-klpk-hd"><div class="wrap"><?php $this->load->view('v1.1/elements/lottery/info_panel', array('noIssue' => true)); ?></div></div>
    ...
    $(function() {
		...
		<?php if ($editballs) {?>
			var balls = [];
			<?php foreach ($editballs as $key => $ballArr)  {?>
			balls[<?php echo $key ?>] = {};
			balls[<?php echo $key ?>].balls = [];
			balls[<?php echo $key ?>].balls[0] = [];
			<?php if (in_array($ballArr[1], array('22', '32', '42', '52', '62'))) {
				$barr = explode('$', str_replace(array('[', ']'), '', $ballArr[0][0]));?>
				balls[<?php echo $key ?>].balls[0]['dan'] = [<?php echo $barr[0]?>];
				balls[<?php echo $key ?>].balls[0]['tuo'] = [<?php echo $barr[1]?>];
				balls[<?php echo $key ?>].playType = 'rx<?php echo substr($ballArr[1], 0, 1)?>dt';
			<?php }else {?>
				balls[<?php echo $key ?>].balls[0]['tuo'] = <?php echo $ballArr[0][0]?>;
				<?php if (in_array($ballArr[1], array(7, 8, 9, 10, 11))) {?>
					balls[<?php echo $key ?>].playType = '<?php echo $playTypeArr[$ballArr[1]]['ename']?>';
				<?php }else {?>
					balls[<?php echo $key ?>].playType = 'rx<?php echo substr($ballArr[1], 0, 1)?>';
				<?php }?>
			<?php }?>
			var p = 1, danl;
			for (i in balls[<?php echo $key ?>].balls) {
				danl = 0;
			...
			balls[<?php echo $key ?>].betMoney = p * 2;
		<?php }?>
