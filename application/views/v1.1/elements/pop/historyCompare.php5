...
$(function(){
	$(".pop-contrast .pop-head h2").html(cnNames[cx._basket_.lotteryId]+'历史开奖对比');
})
$(".pop-contrast").on('click', '.btn-jsq', function(){
	...
	$.ajax({
        type: 'post',
        url:  '/api/'+enNames[cx._basket_.lotteryId]+'compareresult',
        data: {'codes':cx.Lottery.toCastString(cx._basket_.lotteryId, cx._basket_.strings),'issueCount':$(".periods :checked").val()},
        dataType: 'json',
        beforeSend: function(){
        	$(".contrast-bd").empty();
            ...
            for (i in response.data) {
                if (response.data[i].total > 0) {
                    ...
                    if (response.data[i].max == 0) {
                        str += '--';
                    } else if ($.inArray(i, higArr[cx._basket_.lotteryId]) > -1) {
                    ...
                    if (response.data[i].detail.length > 0) {
                        ...
                        $.each(response.data[i].detail, function(j, ele){
                    		...
                    		str += '</div></td><td>奖金：';
                    		if (cx._basket_.lotteryId == 23529 && $.inArray(i, ['3', '4', '5']) > -1) {
                        		var bonarr = [200, 10, 5];
                        		str += bonarr[i-3];
