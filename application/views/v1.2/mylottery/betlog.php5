	...
	<table class="mod-tableA">
		...
		<?php if(!empty($orders)):?>
			...
			<tr>
				...
				<td><a href="/<?php echo in_array($order['lid'], array(44, 45)) ? 'gjc' : BetCnName::getEgName($order['lid']);?>"  target = "_blank"><?php echo BetCnName::getCnName($order['lid']); ?></a></td>
				...
				<?php if($order['status']=='10' && (strtotime("-{$lotteryConfig[$order['lid']]['ahead']} MINUTE", strtotime($order['endTime'])) > time())):?>
					...
					<a class='main-color-s' href="javascript:cx.castCb({orderId:'<?php echo $order['orderId'];?>'}, {ctype:'paysearch', orderType:0,singleFlag:<?php echo $order['singleFlag'];?>});" class="cOrange" style="display:block;">立即支付</a>
