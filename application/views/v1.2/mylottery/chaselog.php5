				...
				<tr>
					...
					<td>
						...
						<?php if($order['status'] == 0 && (strtotime("-{$lotteryConfig[$order['lid']]['ahead']} MINUTE", strtotime($order['endTime'])) > time())) :?>
							<a class='main-color-s' href="javascript:conPay('<?php echo $order['chaseId'];?>',<?php echo $order['singleFlag'];?>);" class="cOrange" style="display:block;">立即支付</a>
...
function conPay( chaseId ,singleFlag)
{    // 根据订单号, 获取订单状态, 信息(类型, 玩法, 期次), 用户剩余金额, 订单金额
    cx.castCb({orderId:chaseId}, {ctype:'paysearch', orderType:1,singleFlag:singleFlag});
}
</script>
	<!--表格 end-->
</div>
