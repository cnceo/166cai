					...
					<td class="s-td">
						<span class="startup">
							...
							<?php if($order['status']=='10' && (strtotime("-{$lotteryConfig[$order['lid']]['ahead']} MINUTE", strtotime($order['endTime'])) > time())):?>
								<a href="javascript:cx.castCb({orderId:'<?php echo $order['orderId'];?>'}, {ctype:'paysearch', orderType:0,singleFlag:<?php echo $order['singleFlag'];?>});" class="btn-ss btn-main">立即支付</a>
							<?php endif;?>
						</span>
					</td>
				</tr>
