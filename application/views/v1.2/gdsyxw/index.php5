<?php 
if(!$lotteryConfig[GDSYXW]['status']) {
	$selling = 0;
}else {
	$selling = 2;
...
<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.2/js/gdsyxw.min.js');?>"></script>
        ...
        <div class="cp-box-bd bet">
            <div class="bet-main">
                    ...
                    <table>
                        ...
                        <?php foreach ($history as $k => $h) {?>
                            <tr>
                            	...
                            	<?php if (isset($info['aIssue']) && $info['aIssue']['awardTime']/1000 > time() && empty($h['awardNum'])) {?>
                            	   <td><em class="main-color-s atime"><?php $rest = $info['aIssue']['awardTime']/1000 - time(); $m = floor($rest/60); $s = $rest - $m * 60; echo str_pad($m, 2, '0', STR_PAD_LEFT).":".str_pad($s, 2, '0', STR_PAD_LEFT)?></em>后开奖...</td>
                            	   <td><span class="main-color-s">--</span></td><td>--</td><td>--</td><td>--</td>
                               	<?php }else if (empty($h['awardNum'])) {?>
	                            ...
	                            <table class="chase-number-table-hd">
	                                <thead>
	                                    ...
	                                    <tr>
	                                        <th>序号</th>
	                                        <th class="tal">
	                                            <input checked type="checkbox">追<input type="text" value="10" data-max="<?php echo count($chases) > 84 ? 84 : count($chases)?>" class="ipt-txt follow-issue">期<s>（共84期）</s>
	                                        </th>
                ...
                <!-- 追号系统 end -->
                        ...
                        <div class="btn-group">
                            <a id="pd_gdsyxw_buy" class="btn btn-main btn-betting <?php echo $showBind ? ' not-bind': '';?>">确认预约</a>
