<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.2/js/lottery.min.js');?>"></script>
...
$sec = ($time % 3600) % 60;
?>
<div class="lotteryTit <?php if (!in_array($lotteryId, array(KS, KLPK, JLKS, JXKS))) {?>issue cp-box-hd<?php } ?>">
    <?php
	...
	<?php elseif ($lotteryId == JLKS): ?>
    	<div class="lottery-info lottery-jlks">
			<div class="lottery-img">
				<svg width="320" height="320">
					<img src="<?php echo getStaticFile('/caipiaoimg/v1.1/images/jlks.png'); ?>" srcset="<?php echo getStaticFile('/caipiaoimg/v1.1/images/jlks.svg'); ?> 2x" width="80" height="80" alt="">
				</svg>
			</div>
			<div class="lottery-info-txt">
				<div class="lottery-info-col lottery-info-col1"><h1 class="lottery-info-name">易快3</h1><p class="lottery-info-num">10分钟一期，轻松中奖~<br>每天8:20-21:40销售</p></div>
				<div class="lottery-info-col lottery-info-time"><span>第<strong></strong>期<b>投注剩余</b></span><span class="time"></span><span class="arrow-tag">剩<?php echo $rest?>期<i></i></span></div>
				<div class="lottery-info-col kj-area">
					<h2 class="kj-periods">第<em><?php echo substr($prev, -3)?></em>期<b>开奖号码</b></h2><div class="kj-num" style="display: none"></div><div class="kj-ing"></div><ul><li>和值：<strong></strong></li><li></li></ul>
				</div>
    ...
    <?php elseif ($lotteryId == JXKS): ?>
    	<div class="lottery-info lottery-ks">
			<div class="lottery-img">
				<svg width="320" height="320">
					<image alt="红快3" xlink:href="<?php echo getStaticFile('/caipiaoimg/v1.1/img/lottery-logo.svg'); ?>" src="<?php echo getStaticFile('/caipiaoimg/v1.1/img/lottery-logo.png'); ?>" width="320" height="320" />
				</svg>
			</div>
			<div class="lottery-info-txt">
				<div class="lottery-info-col lottery-info-col1"><h1 class="lottery-info-name">红快3</h1><p class="lottery-info-num">10分钟一期，轻松中奖~<br>每天08:55-22:55销售</p></div>
				<div class="lottery-info-col lottery-info-time"><span>第<strong></strong>期<b>投注剩余</b></span><span class="time"></span><span class="arrow-tag">剩<?php echo $rest?>期<i></i></span></div>
				<div class="lottery-info-col kj-area">
					<h2 class="kj-periods">第<em><?php echo substr($prev, -3)?></em>期<b>开奖号码</b></h2><div class="kj-num" style="display: none"></div><div class="kj-ing"></div><ul><li>和值：<strong></strong></li><li></li></ul>
				</div>
	...
	<?php elseif ($lotteryId == CQSSC): ?>
		<div class="lottery-info lottery-cqssc">
                <div class="lottery-img">
                ...
                <div class="lottery-info-txt">
                    <div class="lottery-info-top">
                        <h1 class="lottery-info-name">老时时彩</h1>
                        <span class="lottery-info-time">第<b><?php echo $info['cIssue']['seExpect'] ?></b>期</span>
                        <p class="lottery-info-num">10:00-22:00,<b class="main-color-s">10分钟/期</b>；22:00-02:00,<b class="main-color-s">5分钟/期</b></p>
                    </div>
                    <div class="lottery-info-bt">
                        <div class="lnk-list">
                            <a href="/help/index/b5-s17" class="no-bd-l" target="_blank"><i class="icon-font">&#xe620;</i>玩法介绍</a><a href="https://zoushi.166cai.cn/cjwssc/" target="_blank"><i class="icon-font">&#xe622;</i>基本走势</a><a href="javascript:;" target="_self" class="rule"><i class="icon-font">&#xe61a;</i>中奖规则</a>
                        </div>
                    </div>
                </div>
                <div class="kj-area">
                    ...
                    <?php for ($j = 0; $j < 5; $j++) {?>
	                	<div class="kj-num-item">
	                        <div class="inner">
	                            <ul>
	                            <?php switch ($j) {case 0:?><li>正</li><?php break;case 1:?><li>在</li><?php break;case 2:?><li>开</li><?php break;case 3:?><li>奖</li><?php break;case 4:?><li>中</li><?php break;}?>
	                                <?php for ($i = 0; $i <= 9; $i++) {?><li><?php echo $i?></li><?php }?>
	                            </ul>
	                        </div>
	                    </div>
	                <?php }?>
                    </div>
                    <div class="kj-time">销售时间：当日10:00-次日02:00</div>
                </div>
            </div>
            <div class="my-order">
                <div class="my-order-hd">
                    <a href="javascript:">我的投注<i class="arrow"></i></a>
                    <i></i>
                </div>
                <div class="my-order-bd"></div>
            </div>
                    ...
                    success: function (data) {
                        if (isnull(data, false) || isnull(data.issue, true) || isnull(data.seFsendtime, true)) {
                            ...
                            if (time <= 0 && data.restTime > 0) {
                                var tbstr = '', num, j = 0, maxissue, issues = [], multi = parseInt(($(".chase-number-table-hd .follow-multi:first").val() || cx._basket_.multiModifier.value), 10), LASTISSUE = ISSUE;
                                ISSUE = data.issue;
                                $(".curr-issue").html(ISSUE);
                                ...
                                //换期次处理结束
                                $(".pub-pop").remove();
                                cx.Alert({content:"<p class='jiezhi tal'>您好，第<span class='num-red'>"+LASTISSUE+"</span>期方案已超过本期投注截<br>止时间，请尝试购买下一期次。</p>"});
                                chases = data.chases;
                                ...
                                for (i in cx._basket_.chase.chases) {
                                    if (chases[i] === undefined) {
                                        cx._basket_.chase.chaseMulti -= cx._basket_.chase.chases[i].multi;
                                ...
                                for (i in chases) {
                                    if (j < cx._basket_.chase.chaseLength) {
                                        if (cx._basket_.chase.chases[i]) {//用户选了这期
                                            ...
                                            cx._basket_.chase.chaseMulti += multi;
                                            j++;
                                        }
                                        issues.push(i);
                                    }else {
                                ...
                                $('.chase-number-table-hd .follow-issue').val(cx._basket_.chase.chaseLength);
                                ...
                                if ($.inArray(cx.Lottery.lid, [cx.Lid.PLS, cx.Lid.FCSD, cx.Lid.PLW]) > -1) {
