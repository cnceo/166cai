...
var ISSUE = "<?php echo $info['next']['issue']?>", ENDTIME = "<?php echo date('Y-m-d H:i:s', $info['next']['seFsendtime']/1000)?>", type = '<?php echo $fcsdType; ?>', boxCount = <?php echo $boxCount; ?>, 
...
hmselling = '<?php echo $hmselling?>',realendTime = '<?php echo date('Y-m-d H:i:s', $info['next']['seEndtime']/1000)?>', playType = 3; //玩法
var _MplayType = "<?php echo isset($_GET['playType']) ? $_GET['playType'] :''; ?>", MULTI = <?php echo (int)$multi?> || 1;
</script>
<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.1/js/math.min.js');?>"></script>
<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.2/js/fcsd.min.js');?>"></script>
<?php 
                ...
                <div class="pick-area-box bet-tab">
                    ...
                    <div class="bet-tab-hd _Jfc3d">
                        ...
                        <?php foreach ($typeMAP as $k => $type) {?>
                            <li data-type='<?php echo $k?>' <?php if ($k === $fcsdType) {?> class='current' <?php } ?>><a href="/fcsd?playType=<?php echo $k?>"><?php echo $type['cnName']?></a></li>
                        <?php }?>
                        </ul>
                    </div>
                       ...
                       <?php foreach ($miss as $mk => $ms): ?> 
                        <div class="bet-tab-bd-inner tab-radio <?php echo $mk?>" <?php if($fcsdType !== $mk) {echo 'style="display:none"';} ?> >
                            <div class="tab-radio-hd _radio_selected">
                                <ul>
                                    <li class="selected" data-url='fcsd?playType=<?php echo $mk?>' >
                                        <label><input type="radio" name="<?php echo $mk?>_betTypeTab0">普通</label>
                                    </li>
                                    <li class="dssc" data-url='pls/dssc/<?php echo $mk?>'>
                            ...
                            <div class="tab-radio-bd">
                                  ...
                                  <!--直选-->
                                  <?php if ($mk == 'z6'): ?>
                                  <div class="tab-radio-inner _z6Inner">
                                      <div class="pick-area">
                                          <p class="pick-area-explain"><i class="icon-font">&#xe61b;</i>玩法说明：上传方案中的单式号码与开奖号码相同（顺序不限）即中173元！</p>
                                      </div>
                                      <div class="upload">
                                          <div class="rule">
                                              <h3>上传规则：</h3>
...
<script type="text/javascript" src="<?php echo getStaticFile('/caipiaoimg/v1.2/js/fc3d_pl3_comm.min.js');?>"></script>