    ...
    $('.league').click(function() {
        ...
        $('.match').each(function(key, match) {
            ...
            if (league == val) {
                $match.toggle();
                $match.next().toggle();
                if ($match.find('.open-cbf').hasClass('opened')) {
                    $match.find('.open-cbf').trigger('click');
                }
                if ($match.find('.open-jqs').hasClass('opened')) {
                    $match.find('.open-jqs').trigger('click');
                }
                if ($match.find('.open-bqc').hasClass('opened')) {
                    $match.find('.open-bqc').trigger('click');
                }
            }
        });
        onScroll();
    ...
    <div class="userLottery">
        ...
        <table class="lotteryTableCP matches">
            <tbody>
                ...
                <tr>
                    <td colspan="5" style="text-align: left; padding-left: 10px; font-weight: bold;"><?php echo $date; ?></td>
                </tr>
                ...
                <tr
                    ...
                    <td width="25%" rowspan="2"><?php echo $match['home']; ?></td>
                    <td width="25%" rowspan="2"><?php echo $match['awary']; ?></td>
                    <td width="25%" rowspan="2"><span class="<?php if ($match['bfGd']): ?>open-cbf<?php endif; ?>">展开投注项</span></td>
                </tr>
                <tr class="rqspf-detail">
                    <td width="10%">比赛：<?php echo date('m-d H:i', $match['dt'] / 1000); ?></td>
                </tr>
              	<tr class="cbf-options hidden">
                    <td colspan="12" class="border0">
                        <ul class="lotteryTfootUl">
                            <?php foreach ($cbfWinOptions as $key => $value): ?>
                            <li class="cbf-option" data-val="<?php echo $key; ?>" data-odd="<?php echo $match['bfSp' . $key]; ?>">
                                <p><?php echo $value; ?></p>
