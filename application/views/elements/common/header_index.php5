<?php if (!$this->is_ajax): ?>
    ...
    <html>
        <head>
            ...
            $title = str_replace(array('#cnName#', '*date*', '#pageNumber#','#pageTitle#'), array($cnName, (!empty($date) ? date('Y-m-d', $date) : ''), (($pageNumber > 1) ? '-第' . $pageNumber . '页' : ''),(!empty($pageTitle)?$pageTitle:'')), $set_data['title']);
            $keywords = ($pageNumber > 1) ? '' : str_replace(array('#cnName#', '*date*'), array($cnName, (!empty($date) ? date('Y-m-d', $date) : '')), $set_data['keywords']);
            $description = ($pageNumber > 1) ? '' : str_replace(array('#cnName#', '*date*'), array($cnName, (!empty($date) ? date('Y-m-d', $date) : '')), $set_data['description']);
            ...
            <meta name="baidu-site-verification" content="qy8onkL15X" />
