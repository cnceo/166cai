...
class Lottery_Model extends MY_Model
{
    ...
    public function getDetail($lotteryId, $issue = null)
    {
        ...
        if (in_array($lotteryId, array('21406', '21407', '53', '56', '57', '21408', '54', '55', '21421'))) 
        {
            $sql .= " and DATE(award_time)= ? and delect_flag = 0";
            return $this->slaveCfg1->query($sql, array($issue))->getAll();
        } 
        elseif ($issue) 
        {
        ...
        return $this->slaveCfg1->query($sql,array($issue))->getRow();
    }
    ...
    {//已售
        ...
        elseif (in_array($enName, array('syxw', 'jxsyxw', 'ks', 'hbsyxw', 'klpk', 'cqssc','jlks','jxks', 'gdsyxw')))
        {
        ...
        $result = $this->slaveCfg1->query($sql)->getRow();
     	if(empty($result) && in_array($enName, array('syxw', 'jxsyxw', 'ks', 'jlks', 'jxks', 'hbsyxw', 'klpk', 'cqssc', 'gdsyxw')))
        {
    ...
    public function getTeamByIssue($mid)
    {
        ...
        return $this->slaveCfg1->query($sql,array($mid))->getAll();
    }
	...
	public function getKjinfo($lotteryId)
    {
    	...
    	$str = '';
    	if(in_array($lotteryId, array(self::SYYDJ, self::JXSYXW, self::KS, self::JLKS, self::JXKS, self::HBSYXW, self::KLPK, self::CQSSC, self::GDSYXW)))
    	{
    		$str = '_TZ';
    	}
    	...
    	if(!in_array($lotteryId, array(self::SYYDJ, self::JXSYXW, self::KS, self::JLKS, self::JXKS, self::HBSYXW, self::KLPK, self::SSQ, self::CQSSC, self::GDSYXW))) {
    ...
    public function refreshSyxwAwards()
    {
        ...
        $awards = $this->slaveCfg1->query($sql)->getAll();
        if(!empty($awards))
        {
            $this->load->driver('cache', array('adapter' => 'redis'));
            $REDIS = $this->config->item('REDIS');
    ...
    {
        $sql = "select issue,sale_time,end_time,award_time,awardNum,sale,pool from cp_cqssc_paiqi where 1 and status >= 50 and delect_flag = 0 order by issue desc limit 100";
        $awards = $this->slaveCfg1->query($sql)->getAll();
        if(!empty($awards))
        {
            $this->load->driver('cache', array('adapter' => 'redis'));
