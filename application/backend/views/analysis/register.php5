...
?>
<div class="path">您的位置：
    <a href="http://caipiao.2345.com/backend/Analysis/">数据分析 </a>&nbsp;&gt;&nbsp;
    <a href="http://caipiao.2345.com/backend/Analysis/register">注册</a></div>
<div class="data-table-filter mt10">
    <form action="/backend/Analysis/register/" method="get" id="registerForm">
        <table>
            <colgroup>
                <col width="150">
                <col width="150">
    ...
    <table>
        ...
        <tr>
            <?php foreach ($dailyFieldsMap as $fieldName): ?>
                <th>
                    <?php echo (in_array($fieldName, array('注册转化率', '有效用户转化率'))
                            ? ''
                            : '日均')
                        . $fieldName; ?>
                </th>
...
<script>
    $(function () {
        ...
        $("#csvDownload").click(function () {
            tableExport('statTable', '注册数据统计', 'csv');
        });
