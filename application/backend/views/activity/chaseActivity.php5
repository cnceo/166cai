...
?>
<div class="path">您的位置：运营活动&nbsp;>&nbsp;<a href="/backend/Activity/chaseActivity">数字彩活动</a></div>
<div class="mod-tab-hd mt20">
	<ul>
    	<li class="current"><a href="/backend/Activity/chaseActivity">追号不中包赔</a></li>
	</ul>
</div>
<div class="data-table-filter mt10" style="width:960px">
  <form action="/backend/Activity/chaseActivity" method="get"  id="search_form_order">
  <table>
    <colgroup>
      <col width="62" />
      <col width="232" />
    ...
    <th class="tar">套餐：</th>
    <td>
    	<select class="selectList w130" name="chaseType">
      		<?php foreach ($chaseTypes as $key => $val):?>
      		<option value="<?php echo $key;?>" <?php if($search['chaseType'] == $key): echo "selected"; endif;?>><?php echo $val;?></option>
      		<?php endforeach;?>
      	</select>
      </td>
      <td>
      <label for="notBonus"><input id="notBonus" name="notBonus" value='1' type="checkbox" class="ckbox"  <?php if($search['notBonus'] == 1): echo "checked"; endif;    ?> />未中奖</label>
      </td>
      <td >
...
<div class="data-table-list mt20">
  <table>
    ...
    <tr>
        ...
        <td><a href="/backend/Management/chaseDetail/?id=<?php echo $order['chaseId']; ?>" class="cBlue" target="_blank">查看</a>
    </tr>
<?php endforeach; ?>
    </tbody>
    <tfoot>
