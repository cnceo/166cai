...
<div class="mod-tab mt20">
  	<div class="mod-tab-hd">
    	<ul>
      		...
      		<li class="current"><a href="/backend/Match/jclq">竞彩篮球</a></li>
  	...
  	<div>
  		<div class="data-table-filter">
  			<form action="/backend/Match/jclq" method="get"  id="search_form">
          		<table>
            		<tbody>
            			<tr>
              				<td>
	            	...
	            	<tbody>
	              		...
	              		<tr id="<?php echo $row['id'];?>">
	              			<td><input type="checkbox" class="ck_" data-index="<?php echo $row['id'];?>"></td>
	               			...
	               			<td><?php if($row['m_status'] == 0): echo isset($score[1]) ? $score[1] : ''; else: echo '----'; endif;?></td>
	               			<td><?php if($row['m_status'] == 0): echo $score[0]; else: echo '----'; endif;?></td>
	               			<td><?php echo ($row['cstate'] & 1) ? "是" : "否"; ?></td>
	               			<td><?php if($row['m_status'] == 0): echo "否"; else: echo "是"; endif;?></td>
	               			<td><?php echo getJcStatus($row['begin_time'], $row['status'], $row['aduitflag']);?></td>
	                ...
	                <table>
	                    <thead>
	                        <tr>
	                            <th width="56">比赛场次</th>
	                            <th width="120">对阵</th>
	                            <th width="110" class="tac">客队比分</th>
	                            <th width="110" class="tac">主队比分</th>
	                        </tr>
	                    </thead>
...
$(function(){
	...
	$("#cancel").click(function(){
		...
		var title = td.eq(1).html() + " " + td.eq(4).html() + " vs " + td.eq(3).html();
		$("#pop_name").html(title);
		$("#cancelId").val(id);
		popdialog("cancelPop");
	});
	...
	$("#cancelDelaySubmit").click(function(){
		$.ajax({
            type: "post",
            url: '/backend/Match/jclq_cancel_delay',
            data: $("#delayForm").append("<input type='hidden' name='env' value='<?php echo ENVIRONMENT?>'>").serialize(),
            success: function (data) {
                var json = jQuery.parseJSON(data);
                alert(json.message)
	...
	$("#score").click(function(){
		...
		var title = td.eq(1).html() + " " + td.eq(4).html() + " vs " + td.eq(3).html();
		var thead = '<tr><td></td><td>'+ td.eq(4).html() + '</td><td>' + td.eq(3).html()+'</td></tr>';
		var html  = '<tr><td>比分</td><td><input type="text" class="ipt" name="full_a" value="'+td.eq(7).html()+'" /></td><td><input type="text" class="ipt" name="full_h" value="'+td.eq(6).html()+'" /></td></tr>';
		$("#thead2").html(thead);
		$("#tbody2").html(html);
	...
	$(".captureDetail").click(function(){
		...
		$.ajax({
            type: "post",
		    url: "/backend/Match/getJclqCapture",
		    data: {'mid':mid},
		    success: function(data){
		    	var json = jQuery.parseJSON(data);
                if(json.status == '1'){
	...
	$("#detailSubmit").click(function(){
		...
		$.ajax({
            type: "post",
		    url: "/backend/Match/saveJclqCapture",
	...
	$("#aduitCheck").click(function(){
		...
		for (var i = 0; i < ids.length; ++i) {
			...
			if(td.eq(11).data('status') == '2')
			{
				...
				if(hs !== '')
				{
					hscore = PrefixInteger(hs, 3);
				}
				var as = td.eq(7).html();
				if(as !== '')
				{
					ascore = PrefixInteger(as, 3);
				}
			}

			tpl += '<tr>';
			tpl += '<td>' + td.eq(1).html() + '</td>';
			tpl += '<td>' + td.eq(4).html() + ' VS ' + td.eq(3).html() + '</td>';
			tpl += '<td class="tac"><input type="text" class="ipt w84 tac" value="' + ascore + '" maxlength="3" data-index="1" name="score[' + id + '][ascore]"></td>';
			tpl += '<td class="tac"><input type="text" class="ipt w84 tac" value="' + hscore + '" maxlength="3" data-index="2" name="score[' + id + '][hscore]"></td>';
			tpl += '</tr>';
		}
	...
	$("#verifySubmit").click(function(){
		...
		$.ajax({
            type: "post",
		    url: "/backend/Match/verifyJclqScore",
