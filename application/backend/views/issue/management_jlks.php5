			  ...
			  <div class="pop-body">
				  <div class="data-table-filter del-percent">
					  <table>
						  ...
						  <tfoot>
						  <tr>
							  <td colspan="2"><p>输入开奖号码格式为1、2</p></td>
						  </tr>
						  <tr>
                          ...
                          <tr>
                              <td>开奖号码：</td>
                              <td>
                                  <div class="num-box" id ='num-box1'>
                                      <input name="awardnum[0]" type="text" value="" maxlength="1">
                                      <input name="awardnum[1]" type="text" value="" maxlength="1">
                                      <input name="awardnum[2]" type="text" value="" maxlength="1">
                                  </div>
                              </td>
                          </tr>
                          <tr>
                              ...
                              <td>
                                  <div class="num-box">
                                      <input name="awardnumAgain[0]" type="text" value="" maxlength="1">
                                      <input name="awardnumAgain[1]" type="text" value="" maxlength="1">
                                      <input name="awardnumAgain[2]" type="text" value="" maxlength="1">
                                  </div>
                              </td>
                          </tr>
						  </tbody>
...
<script>
var type = '<?php echo $search['type'];?>', cnameArr = {'ks':'上海快三', 'jlks':'吉林快三', 'jxls':'江西快三'}
$(function(){
    ...
    $("#recountSubmit").click(function(){
      ...
      $.ajax({
            ...
            success: function (data) {
              ...
              if(data.status == 'y'){
                closePop();
                layer.alert('操作成功~', {icon: 1,btn:'',title:'温馨提示',time:0});
              }else{  
...
$("#modifyIssue").click(function(){
	...
	if(!id)
	{
    layer.alert('请选中要修改的期次~', {icon: 2,btn:'',title:'温馨提示',time:0});
		return false;
	}
	var status = $(".select").find('.status').html();
	if(status != "截止")
	{
    layer.alert('仅对截止的期次进行添加开奖号码操作~', {icon: 2,btn:'',title:'温馨提示',time:0});
		return false;
	}
	var data  = $(".select").find('.awardNum').html();
	var title = cnameArr[type]+" 第" + id + "期开奖号码";
...
$("#issueSubmit").click(function(){
  layer.load(0, {shade: [0.5, '#393D49']});
	$.ajax({
      ...
      if(json.status =='y')
      {
        layer.alert('操作成功~', {icon: 1,btn:'',title:'温馨提示',time:0,end:function(){location.reload();} });
      }else{
...
$("#issuesSubmit").click(function(){
    ...
    if(issues.length < 1)
    {
      layer.alert('请先选择一次比赛~', {icon: 2,btn:'',title:'温馨提示',time:0});
      return false;
    }
    else
    {
...
$("#deleteIssue").click(function(){
	issues = [];
	$(".ck_").each(function(){
		if(this.checked){issues.push($(this).val());}
	})
  if(issues.length < 1)
  {
    layer.alert('请先选择你要删除的期次~', {icon: 2,btn:'',title:'温馨提示',time:0});
    return false;
  }
  else
  {
...
$('.num-box').on('keyup', 'input', function(){
    if(!/\D/.test($(this).val())){
        if(($(this).val().length == 1)){
            if($(this).index()==2 && $(this).parents('.num-box').attr('id')=='num-box1')
            {
              $('.num-box').eq(1).find('input:first').focus();
            }
