            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="ipt w150" name="3dj_dzjj" value="<?php echo $bonusDetail['3dj']['dzjj'];?>"></td>
                    </tr>
                    <tr>
                        <td>四等奖</td>
                        <td><input type="text" class="ipt w150" name="4dj_zs" value="<?php echo $bonusDetail['4dj']['zs'];?>"></td>
                        <td><input type="text" class="ipt w150 none_input" readonly="true" name="4dj_dzjj" value="<?php echo $bonusDetail['4dj']['dzjj'];?>"></td>
                    </tr>
                    <tr>
                        <td>五等奖</td>
                        <td><input type="text" class="ipt w150" name="5dj_zs" value="<?php echo $bonusDetail['5dj']['zs'];?>"></td>
                        <td><input type="text" class="ipt w150 none_input" readonly="true" name="5dj_dzjj" value="<?php echo $bonusDetail['5dj']['dzjj'];?>"></td>
                    </tr>
                    <tr>
                        <td>六等奖</td>
                        <td><input type="text" class="ipt w150" name="6dj_zs" value="<?php echo $bonusDetail['6dj']['zs'];?>"></td>
                        <td><input type="text" class="ipt w150 none_input" readonly="true" name="6dj_dzjj" value="<?php echo $bonusDetail['6dj']['dzjj'];?>"></td>
                    </tr>
                    <tr>
                        <td>七等奖</td>
                        <td><input type="text" class="ipt w150" name="7dj_zs" value="<?php echo $bonusDetail['7dj']['zs'];?>"></td>
                        <td><input type="text" class="ipt w150 none_input" readonly="true" name="7dj_dzjj" value="<?php echo $bonusDetail['7dj']['dzjj'];?>"></td>
                    </tr>
                </tbody>
            </table>
...
<script>
    ...
    $("#submit").click(function(){
        ...
        var zs7 = $('input[name="7dj_zs"]').val();
        var dzjj7 = $('input[name="7dj_dzjj"]').val();
        var reg = /^[0-9]\d*$/;
        if (issue !== '<?php echo $issue;?>') {
        	layer.alert('期次信息有误，请确认后填写', {icon: 2,btn:'',title:'温馨提示',time:0});
            return false;
        }
        if(!reg.test(sale) || !reg.test(pool) || !reg.test(zs1) || !reg.test(dzjj1) || !reg.test(zs2) || !reg.test(dzjj2) || !reg.test(zs3) || !reg.test(dzjj3) || !reg.test(zs4) || !reg.test(dzjj4) || !reg.test(zs5) || !reg.test(dzjj5) || !reg.test(zs6) || !reg.test(dzjj6) || !reg.test(zs7) || !reg.test(dzjj7))
        {
            layer.alert('修改内容必须为非负整数~', {icon: 2,btn:'',title:'温馨提示',time:0});
            return false;
        }
        var awardNum = $('input[name="awardNum"]').val();
        awardNum = awardNum.replace('(',',');
        awardNum = awardNum.replace(')','');
        ball = awardNum.split(',');
        if(ball.length != 8)
        {
            layer.alert('请输入7个开奖号码和1个特殊号码~', {icon: 2,btn:'',title:'温馨提示',time:0});
            return false;
        }
        for(var i in ball)
        {
            ...
            if(ball[i] < 1 || ball[i] > 30)
            {
                layer.alert('开奖号码范围为1-30~', {icon: 2,btn:'',title:'温馨提示',time:0});
                return false;
            }
        }
