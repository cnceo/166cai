...
<div class="data-table-list mt20">
  <table>
    ...
    <tr>
      <td><a target="_blank" href="/backend/Order/order_detail/?id=<?php echo $check['id']; ?>" class="cBlue"><?php echo $check['orderId']  ?></a></td>
      <td><a target="_blank" href="/backend/User/user_manage/?uid=<?php echo $check['uid'] ?>" class="cBlue"><?php echo $check['userName']  ?></a></td>
      ...
      <td><?php if($fromType != 'ajax'): ?><a href="" class="cBlue dc-success " data-id="<?php  echo $check['orderId']  ?>" >执行派奖</a><a href="" class="cBlue dc-user-success mlr10" data-id="<?php  echo $check['orderId']  ?>">用户已领</a><a href="" class="cBlue dc-fail mlr10" data-id="<?php  echo $check['orderId']  ?>">派奖失败</a><?php endif; ?></td>
    </tr>
    <?php endforeach; ?>
    </tbody>
    <tfoot>
