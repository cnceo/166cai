...
<div class="frame-container" style="margin-left:0;">
    ...
    <div class="mod-tab-hd mt20">
        <ul>
            <li class="current"><a href="/backend/Management/manageKind/">彩种管理</a></li>
            <li><a href="/backend/Management/manageHemai/">合买彩种管理</a></li>
        </ul>
    </div>
    <div class="mt10">
                ...
                <?php if ($configItems): ?>
                    ...
                    <?php foreach ($configItems as $row): ?>
                        <tr data-issue="<?php echo $row['lotteryId']; ?>"
                            ...
                            data-window="<?php echo $row['window']; ?>"
                            data-ahead="<?php echo $row['ahead']; ?>">
                            <td><?php echo in_array($row['lotteryId'], array(BetCnName::RJ, BetCnName::SFC))
                                    ? '胜负彩/任九' : BetCnName::$BetCnName[$row['lotteryId']]; ?></td>
                            ...
                            <td><?php if ($row['window']) {
                                    echo $row['window'];
                                }
                                else {
                                    echo "--";
                                } ?></td>
                            <td><?php echo $row['ahead']; ?></td>
...
<form id="modifyForm" method="post" action="/backend/Management/configLottery">
    <!-- 修改 start -->
            ...
            <div class="pop-body">
                <div class="data-table-filter del-percent">
                    ...
                    <table>
                        ...
                        <tr>
                            <th>开放期次：</th>
                            <td><input type="text" value="" class="ipt w222" id="windowId" name="window"></td>
                        </tr>
                        <tr>
                            <th>提前截止时间：</th>
                            <td><input type="text" value="" class="ipt w222" id="aheadId" name="ahead"></td>
                        </tr>
    ...
    $(function () {
        ...
        $("tr").click(function () {
            ...
            if ($(this).attr("data-issue") != null && $(this).attr("data-id") != null) {
                ...
                $("#aheadId").val($(this).attr("data-ahead"));
            }
        });
        ...
        $("#modifySubmit").click(function () {
            ...
            var window = $("#windowId").val();
            var ahead = $("#aheadId").val();
            $.ajax({
                type: "post",
                url: "/backend/Management/configLottery",
                data: {
                    'id': id,
                    'status': status,
                    'window': window,
