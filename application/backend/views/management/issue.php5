                ...
                <?php if ($result): ?>
                    ...
                    <?php foreach ($result as $row): ?>
                        <tr data-issue="<?php echo $row['issue']; ?>" data-status="<?php echo $row['status']; ?>">
                            ...
                            <td>
                                <?php if ( ($row['status'] == '已过关' || $row['rstatus'] == $this->order_status['paiqi_jjsucc']) && !in_array($search['type'],array('ssq','dlt','qxc','qlc','fc3d','pl3','pl5'))): ?>
                                   <a
                                    onclick="javascript:recalculateAlert(<?php echo "'" . $row['issue'] . "','" . $search['type'] . "'"; ?>);"
                                    href="javascript:void(0);" class="btn-blue">重算奖金</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
