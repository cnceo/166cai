            ...
            foreach ($details['datas'] as &$order) {
                ...
                if ($order['ctype'] == 1) {
                    ...
                    if ($order['status'] == 3 || $order['status'] == 4) {
                        $order['tradeDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/hemai/detail/hm' . $order['orderId']
                                . '/' . urlencode($token);
                    } else {
                    ...
                    $order['tradeDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/hemai/gdetail/gd' . $order['orderId']
                            . '/' . urlencode($token);                    
                } else {
        ...
        if (!empty($orders)) {
            ...
            foreach ($orders as $key => $items) {
                ...
                if (!in_array($items['orderType'], array(41, 42))) {
                    ...
                    $orderInfo[$key]['orderDetailUrl'] = $protocol.$this->config->item('pages_url').'ios/hemai/detail/hm'.$items['orderId'].'/'.urlencode($token);
                }
            }
        }
