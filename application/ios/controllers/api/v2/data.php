...
class Data extends MY_Controller
{
	...
	{
		$info = $this->Cache->getIndex($platform = 'ios');
		// 请求头信息
		$headerInfo = $this->getRequestHeaders();
		$channel = $this->recordChannel($headerInfo['channel']);
		// 按渠道处理
	...
	public function getLotterys($info)
	{
		...
		if(!empty($info))
		{
			...
			foreach ($info as $key => $items) 
			{
				if(!empty($items['subs']))
				{
					...
					{
					    //小于3.3版本预埋彩种不返回
					    if($headerInfo['appVersionCode'] < '3030001' && in_array($val['lid'], array('57', '21421'))) {
					        unset($info[$key]['subs'][$k]);
					        continue;
					    }
		...
		if($headerInfo['appVersionCode'] < '3030001') {
		    foreach ($info as $key => $items) {
		        if(!empty($items['subs'])) {
		            $info[$key]['subs'] = array_values($items['subs']);
		        }
		    }
		}
