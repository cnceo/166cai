...
//        $data = array('order' => '40', //(合买战绩 21升序,20降序)(进度 00降序,01升序)(金额 30降序,31升序)(截止时间 40降序,41升序)
//            'state' => '0',
//            'money' => '0',
//            'page' => '1', //第几页
//            'number' => '10', //一页几个
...
//            'uid' => ''//当前用户token); 
        ...
        if(empty($orders['data']) && !empty($data['uname']) && $headerInfo['appVersionCode'] >= '2070001')
        {
            // 精确匹配用户
...
//            'userName' => 'yidefu',//用户名
//            'buyPlatform' => '1',//固定值
//            'codes' => '01,06,16,17,30,32|09:1:1',//投注号
//            'lid' => '51',//彩种id
//            'money' => 2,//金额
//            'multi' => 1,//固定值
//            'issue' => 2017024,//期次
...
//        );
        ...
        // 1.0版本关闭支付充值
        if ($headerInfo['appVersionCode'] < '3') {
            $result = array(
                'status' => '0',
        ...
        // 竞彩足球暂停销售
        if ($data['lid'] == '42' && $headerInfo['appVersionCode'] < '4') {
            $result = array(
                'status' => '0',
                'msg' => '暂停销售，请前往网站购彩。' . $this->config->item('domain'),
                'data' => '',
            );
            echo json_encode($result);
            exit();
        ...
        if (in_array($data['lid'], array('42', '43'))) {
            // 去除空格
            $data['codecc'] = trim($data['codecc']);
            if (!isset($data['codecc']) || $data['codecc'] === '') {
                $result = array(
                    'status' => '0',
                    'msg' => '缺少必要参数',
                    'data' => '',
        ...
        if (!empty($issueInfo['aIssue']) && in_array($data['lid'], array('51', '23529', '35', '10022', '23528')) && time() > (floor($issueInfo['aIssue']['seEndtime'] / 1000) - $lotteryConfig[$data['lid']]['ahead'] * 60) && time() < floor($issueInfo['aIssue']['seEndtime'] / 1000)) {
            $result = array(
                'status' => '0',
    ...
    public function orderComplete($data, $orderType = 0)
    {
        ...
        $payView = $protocol.$this->config->item('pages_url').'ios/order/doPay/'.urlencode($orderDetail);
...
//            'uid' => 1024,//uid
//            'buyPlatform' => '1',//固定值
//            'orderId' => '20170303102823995478',//orderid
//            'money' => 2,//金额
//        );
        $params = array(
            'ctype' => '',
    ...
    public function getUserRecord()
    {
        ...
        if(!empty($uinfo))
        {
            ...
            $records = array(
                ...
                'united_points' =>  $info ? $info['united_points'] : 0,
            );
            if ($data['puid']) {
