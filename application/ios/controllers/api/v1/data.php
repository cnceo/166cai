...
class Data extends MY_Controller
{
	...
	public function lotteryInfo()
	{
		...
		$linfo = $this->Data->getLotteryInfo('ios');
		...
		if(!empty($linfo))
		{
			...
			foreach ($linfo as $key => $items)
			{
				...
				// https
				$items['logUrl'] = ParseHttp($items['logUrl']);
	...
	public function adInfo()
	{
		...
		if(!empty($info))
		{
			...
			foreach ($info as $key => $items) 
			{
				if(!empty($items['channels']) && in_array($this->recordChannel($this->headerInfo['channel']), explode(',', $items['channels'])))
				{
					$items['imgUrl'] = ParseHttp($items['imgUrl']);
					$items['hrefUrl'] = ParseHttp($items['hrefUrl']);
					unset($items['channels']);
	...
	public function awardsInfo()
	{
		...
		if (!empty($awardsInfo)) 
		{
			...
			foreach ($awardsInfo as $key => $items) 
			{
				...
				// 福彩3D 排列三 新增形态
				$awardsInfo[$key]['pattern'] = (in_array($lotteryId, array('52', '33', '55'))) ? $this->comm->getNumberPattern($lotteryId, $items['awardNumber']) : '';
			}
	...
	public function getCurrentByLid()
	{
		...
		if(!empty($info[0]))
		{
			...
			foreach ($info as $key => $items) 
			{
				...
				$lotteryInfo[$key]['seLotName'] = BetCnName::$BetCnName[$items['seLotid']];
				$lotteryInfo[$key]['awardNumber'] = $items['awardNumber'];
				...
				$lotteryInfo[$key]['awardPool'] = $awardInfo[$lid]['awardPool'];
	...
	public function getJjcInfo()
	{
				...
				case '42':
					...
					foreach($info as $key => $items)
					{
						...
						else
						{
							...
							$match[$key]['away'] = $items['awarySname'] ? $items['awarySname'] : $items['awary'];
							...
							$match[$key]['hot'] = $items['hot'] ? intval($items['hot']) : 0;
							// 竞彩数据汇总
                    ...
                    if($headerInfo['appVersionCode'] >= '2070001')
                    {
                    	array_multisort($hotArr, SORT_ASC, $keyArr, SORT_ASC, $matches);
                    }
                    $matches = $this->getMatchFormat($matches);
                    break;
                // 竞彩篮球
                case '43' :
                	// 获取竞彩对阵数据 比赛编号 历史交锋
		...
		if($lid == '43' && $headerInfo['appVersionCode'] >= '22')
		{
			$result = array(
				'status' => '1',
				'msg' => 'Success',
	...
	public function getAwards()
	{
		...
		if(!empty($awardData))
		{
    		...
    		{
    			$award['seLotid'] = (string)$award['seLotid'];
    			$award['seLotName'] = (string)BetCnName::$BetCnName[$award['seLotid']];
    			$award['seFsendtime'] = (string)$award['seFsendtime'];
    			$award['seEndtime'] = (string)$award['seEndtime'];
    			$award['awardTime'] = (string)$award['awardTime'];
    			$award['sale_time'] = (string)$award['sale_time'];
    			$award['seDsendtime'] = (string)$award['seDsendtime'];
			...
			if($headerInfo['appVersionCode'] <= '7')
			{
				$lotteryInfo = array('51','23529','52','42','43','21406','33','35','11','19');
			}
			elseif($headerInfo['appVersionCode'] < '2070000')
			{
				$lotteryInfo = array('51','23529','52','42','43','21406','21407','21408','33','35','11','19','53');
			}
			else
			{
			    $lotteryInfo = array('51', '23529', '52', '42', '43', '21406', '21407', '21408', '33', '35', '11', '19', '53', '10022', '23528', '55', '57', '21421');
			}
	...
	public function cityInfo()
	{
		...
		{
			$cityList = array();
			foreach ($info['province'] as $k => $list) 
			{
				$cityList[$k]['province'] = $list['province'];
				foreach ($info['details'] as $key => $items) 
				{
					if($list['province'] == $items['province'])
					{	
						$cityList[$k]['city'][] = $items['city'];
					}
				}
			}
	...
	private function getMissNumber($lid)
	{
		...
		if(!empty($info[0]['awardNumber']) && !empty($missInfo))
		{
			...
			if($lastMiss >= $issue)
			{
				...
				elseif (in_array($lid, array('53', '57')))
				{
	...
	public function checkVersion()
	{
		...
		// $versionData = $this->config->item('versionData');

		$versionData = $this->Cache->getAppConfig('ios');
		ksort($versionData);
		...
		{
			foreach ($versionData as $key => $items) 
			{
				if($items['isCheck'] == 0)
				{
					array_push($versionNew, $items);
				}
				$versionConfig[$items['versionCode']] = $items;
			}
		}
		...
		if(!empty($versionConfig[$appVersionCode]))
		{
			$current = array(
				'showAlert' => $versionConfig[$appVersionCode]['showAlert'],
				'showRedpack' => $versionConfig[$appVersionCode]['showRedpack']
			);
		}

		// 获取最新版本的跟新信息
		$versionData = array_pop($versionNew);

		// 最新版本的更新信息
		$newConfig = array(
			'appVersionName' => $versionData['versionName'],
			'appVersionCode' => $versionData['versionCode'],
			'needUpgrade' => $this->checkUpgrade($appVersionCode, $versionConfig, $versionData),									// 检查当前版本是否需要强制升级
			'versionInfo' => $versionData['mark'],
		...
		if($appVersionCode < $versionData['versionCode'] && $versionData['isCheck'] == '0')
		{
			$result = array(
				'status' => '1',
				'msg' => '新升级信息',
	...
	/*
 	 ...
 	 * @date:2016-03-23
 	 */
	public function checkUpgrade($appVersionCode, $versionConfig, $versionData)
	{
		$needUpgrade = '0';
		if(!empty($versionConfig[$appVersionCode]['versionName']))
		{
			$versionName = $versionConfig[$appVersionCode]['versionName'];

			if(!empty($versionData['upgradeVersion']))
			{
				$upgradeArry = explode(',', $versionData['upgradeVersion']);
				if(in_array($versionName, $upgradeArry))
				{
					$needUpgrade = '1';
				}
			}
	...
	public function preload()
	{
		...
		$info = $this->Cache->getBannerInfo(2, 1, $platform = 'ios');
		...
		if(!empty($info))
		{
			for ($i = 0; $i < 3; $i++) 
			{ 
				if(!empty($info[$type]['channels']) && in_array($this->recordChannel($this->headerInfo['channel']), explode(',', $info[$type]['channels'])) && !empty($info[$type]['imgUrl']))
				{
					// 启动页竞彩足球区分玩法
					$playTypeName = '';
					if($info[$type]['lid'] == '42' && !empty($info[$type]['extra']))
                	{
                		$extra = json_decode($info[$type]['extra'], true);
					...
					$preloadInfo = array(
						'url'		=>	(strpos($info[$type]['imgUrl'], 'http') !== FALSE) ? $info[$type]['imgUrl'] : $this->config->item('protocol') . $info[$type]['imgUrl'],
						'hrefUrl'	=>	$info[$type]['url'],
						'lid'		=>	$info[$type]['lid'] ? $info[$type]['lid'] : 0,
	...
	/*
 	 ...
 	 * @date:2015-08-05
 	 */
	public function refreshLottery($platform)
	{
		$this->Data->freshLotteryInfo($platform);
		$this->Data->freshAddInfo($platform);
	}
	...
	private function getActivityMark($lid)
	{
		...
		$banner = $this->Cache->getBannerInfo(5, 1, $platform = 'ios');
		if(!empty($banner))
		{
	...
	public function loading()
	{
		...
		if(!empty($data['uid']))
		{
			...
			{
			    //小于3.1版本执行
			    if(!empty($headerInfo['deviceId']) && $headerInfo['appVersionCode'] < '3010001')
			    {
			        // 更新用户登录信息
		...
		$result = array(
			...
			'data' 		=> 	array(
				...
				'lxStatus'	=>	($status['ios'] == 2) ? 0 : 1,
				'lxContent'	=>	($status['ios'] != 2 && $status['ios_content']) ? $status['ios_content'] : '',
	...
	{
		// 获取弹窗信息缓存
		$info = $this->Cache->getBannerInfo(3, 2, $platform = 'ios');
	...
	public function getMatchFormat($matches)
	{
		$matchs = array();
		$matchtpl = array();
		foreach ($matches as $key => $match) 
		{
			$matchtpl[$match['issue']][] = $match;
		}
		ksort($matchtpl);
		foreach ($matchtpl as $issue => $items) 
		{
			$matchs[] = array(
				'weekKey'	=>	$issue,
				'detail'	=>	$items,
	...
	public function getOrderUrl($info)
	{
		...
		if($info['status'] == 1)
		{
			$url .= 'ios/chase/detail/';
		}
		elseif($info['status'] == 3)
		{
			$url .= 'ios/hemai/detail/hm';
		}
		else
		{
			$url .= 'ios/order/detail/';
		}
	...
	{	
		...
		if(!empty($headerInfo['deviceId']))
		{
			...
			$this->api_aso_model->recordIdfa($info);
		}
	}
	...
	{
		$info = $this->Cache->getBannerInfo(4, 2, $platform = 'ios');
	...
	// 注册实名认证引导配置
	public function getbindEmail($checkData = array(), $platform = 'ios')
	{
		$this->load->model('user_model');
		$data = '';
		$info = $this->Cache->getBannerInfo(1, 2, $platform = 'ios');
	...
	public function getRedpackUrl($uid)
	{
        ...
        return $info['data']['ios'] . '/ios/redpack/index/' . urlencode($token);
	}
