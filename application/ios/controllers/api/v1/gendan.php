...
//        );
        ...
        {
            $result = array(
                'status'    =>  ($headerInfo['appVersionCode'] >= '3020001') ? '700' : '300',
                'msg'       =>  '用户未登录',
                'data'      =>  $redata
            );
            die(json_encode($result));
...
//        );
        ...
        if (!$this->checkUserAuth($uinfo, $redata['auth'])) {
            $result = array(
                'status'    =>  ($headerInfo['appVersionCode'] >= '3020001') ? '700' : '300',
                'msg'       =>  '您的登录密码已修改，请重新登录',
                'data'      =>  '',
    ...
    public function orderComplete($data, $orderType)
    {
        ...
        $headerInfo = $this->getRequestHeaders();
        $param['appVersionCode'] = $headerInfo['appVersionCode'];
        // 订单信息加密
        ...
        if ($data['payType'] == 0) {
            ...
            $payView = $protocol . $this->config->item('pages_url') . 'ios/wallet/payComplete/' . urlencode($orderDetail);
        }
