...
class Order extends MY_Controller 
{
	...
	public function creadOrder()
	{
		...
		// log_message('LOG', "创建订单开始 - 请求参数: " . json_encode($data), 'creadOrder');
		$result = array(
			'status' => '0',
			'msg' => '通讯异常',
			'data' => ''
		...
		// 新增预测奖金
		$data['forecastBonus'] = $data['forecastBonus'] ? $data['forecastBonus'] : '';
	...
	public function createChaseOrder()
	{
		$data = $this->strCode($this->input->post('data'));
		$data = json_decode($data, true);
		// log_message('LOG', "创建订单开始 - 请求参数: " . json_encode($data), 'bn_creadOrder');
