...
class Mylottery extends MY_Controller
{
    ...
    public function getMyLotteryList()
    {
        ...
        if(!empty($info['orders']))
        {
            foreach ($info['orders'] as $key => $items) 
            {
                ...
                $lotteryInfo[$key]['ctype'] = $data['ctype'];
        ...
        else
        {
            foreach ($details['datas'] as & $order)
            {
                ...
                if ($order['ctype'] == 1)
                {
                    ...
                    $order['tradeDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/order/detail/' . $order['orderId']
                        . '/' . urlencode($token);
                ...
                elseif(in_array($order['ctype'], array(12, 13)))
                {
                    ...
                    $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https:" : "http:";
                    $order['tradeDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/chase/detail/' . $order['orderId']
                        . '/' . urlencode($token);
                    $order['lid'] = $order['additions'];
                }
                else
                {
                    ...
                    $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https:" : "http:";
                    $order['tradeDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/trade/detail/' . urlencode($tradeToken);
                    $order['lid'] = '0';
                }
            }
    ...
    public function getBetV2($data)
    {
        ...
        if($data['baseType'] == '2')
        {
            ...
            // 自购订单格式处理
            $orderInfo = $this->getChaseListFormat($data['uid'], $odatas['datas'], $orderStatus, $data);
        }
        else
        {
            // 组装查询条件
            ...
            // 自购订单格式处理
            $orderInfo = $this->getBetListFormat($data['uid'], $odatas['datas'], $data);
        }
     ...
     * @date:2016-03-09
     */
    public function getBetListFormat($uid, $orders, $data)
    {
        // 组装数据
        $orderInfo = array();
        ...
        if(!empty($orders))
        {
            ...
            foreach ($orders as $key => $items) 
            {
                ...
                $orderInfo[$key]['orderType'] = $items['orderType'] ? '1' : '0';

                $orderInfo[$key]['baseType'] = $data['baseType'];
                $orderInfo[$key]['subType'] = $data['subType'];
                ...
                $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https:" : "http:";
                $orderInfo[$key]['orderDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/order/detail/' . $items['orderId'] . '/' . urlencode($token);
            }
        }
        return $orderInfo;
    }
     ...
     * @date:2016-03-09
     */
    public function getChaseListFormat($uid, $orders, $orderStatus, $data)
    {
        // 组装数据
        $orderInfo = array();
        ...
        if(!empty($orders))
        {
            ...
            foreach ($orders as $key => $items) 
            {
                $orderInfo[$key]['orderId'] = $items['chaseId'];
                $orderInfo[$key]['lid'] = $items['lid'];
                $orderInfo[$key]['cnName'] = BetCnName::$BetCnName[$items['lid']];
                ...
                $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https:" : "http:";
                $orderInfo[$key]['orderDetailUrl'] = $protocol . $this->config->item('pages_url') . 'ios/chase/detail/' . $items['chaseId'] . '/' . urlencode($token);
            }
        }
        return $orderInfo;
    }
