        ...
        if($returnData['status'])
        {
        	/*if($data['action'] == 1)
        	{
	        	...
	        	$result = array(
					'status' => '1',
					'msg' => '实名信息验证通过',
					'data' => $this->config->item('pages_url') . 'ios/withdraw/phone/' . urlencode($tokenStr) . '/' . $data['bankId']
        ...
        else
        {
            ...
            if ($return['status']) 
            {
                ...
                $result = array(
					'status' => TRUE,
					'msg' => '提现申请成功',
					'data' => $this->config->item('pages_url') . 'ios/withdraw/withdrawComplete/' . urlencode($tokenStr)
				);
            } 
            else 
            {
