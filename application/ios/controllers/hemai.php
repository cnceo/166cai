...
class Hemai extends MY_Controller 
{
	...
	public function detail($orderId, $strCode = null) 
	{
		...
		if (!empty($orderInfo)) 
		{
			...
			$data['appStars'] = $this->checkAppStars($data['joinInfo']);
	...
	*/
    ...
    public function continuebuy($orderInfo, $uid)
    {
    	...
    	$versionInfo = $this->getUserAgentInfo();
    	if( !empty($uid) && ($uid == $orderInfo['uid']) && ((time() >= $orderInfo['endTime']) || ($orderInfo['money'] - $orderInfo['buyTotalMoney']) == 0 || in_array($orderInfo['status'], array(610, 620))) && in_array($orderInfo['lid'], array(51, 23529, 10022, 23528, 52, 33, 35)) && $versionInfo['appVersionCode'] >= 2070001)
    	{
    ...
    public function gdetail($orderId, $strCode = null) 
    {
        ...
        $info = $this->united_planner_model->findByUid($orderInfo['puid'], 'united_points', $orderInfo['lid']);
        $data['orderInfo'] = $orderInfo;
        $data['orders'] = $orders;
    ...
    public function checkAppStars($joinInfo)
    {
    	// 本人
    	if(!empty($joinInfo) && $joinInfo['margin'] > 0)
    	{
    		$result = $this->checkNeedAppStars($joinInfo['margin']);
    	}
    	else
    	{
    		$result = '0';
