    ...
    public function detail($chaseId, $strCode)
    {
        ...
        if(!empty($chaseManageInfo))
        {
            $chaseInfo = array(
                ...
                'appStars' => $this->checkNeedAppStars($chaseManageInfo['info']['bonus']),
            ...
            if(!empty($chaseManageInfo['detail']))
            {
                foreach ($chaseManageInfo['detail'] as $key => $items) 
                {
                    ...
                    $orderList[$key]['orderUrl'] = ($items['status'] < $orderStatus['drawing'] || $items['status'] == $orderStatus['revoke_by_user'] || $items['status'] == $orderStatus['revoke_by_system'] || $items['status'] == $orderStatus['revoke_by_award']) ? '' : $this->config->item('pages_url') . 'ios/order/detail/' . $items['orderId'] . '/' . urlencode($chaseInfo['token']);
            ...
            // 解决福彩3D不显示继续购买
            if($chaseManageInfo['info']['lid'] == Lottery_Model::FCSD && $this->versionInfo['appVersionCode'] <= '14')
            {
            ...
            else
            {
                ...
                {
                    // 是否包含组三单式
                    if(strpos($chaseManageInfo['info']['codes'], ':2:1') !== FALSE && $versionInfo['appVersionCode'] < '3010000')
                    {
                ...
                $chaseInfo['orderPlan'] = array(
                    ...
                    'buyPlatform'   => $chaseManageInfo['info']['buyPlatform'],
                );
            }
        }
        if (!isset($award)) {
