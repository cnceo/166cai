...
<body>
    <div class="wrapper bet-detail hemai-detail bet-detail-<?php echo BetCnName::getEgName($orderInfo['lid']); ?>">
    <!-- 延期公告 -->
                ...
                <table class="table-info">
                    <tbody>
                        ...
                        <tr>
                            <th>发起人</th>
                            <td>
                                <?php if($versionInfo['appVersionCode'] >= 2080001): ?>
                                <!-- 原生查看战绩 -->
                                <a href="javascript:;" onclick="window.webkit.messageHandlers.checkRecord.postMessage({uid:'<?php echo $orderInfo['uid']?>', lid:'<?php echo $orderInfo['zjLid']?>'});" class="name"><?php echo uname_cut($orderInfo['uname'])?></a>
                                <div class="level"><?php echo calGrade($orderInfo['points'], 4)?></div>
                                <?php if($versionInfo['appVersionCode'] >= 3000001): ?>
                                <a href="javascript:;" id="gendan" class="ui-link">定制跟单</a>
                                <?php else: ?>
                                <a href="javascript:;" onclick="window.webkit.messageHandlers.checkRecord.postMessage({uid:'<?php echo $orderInfo['uid']?>', lid:'<?php echo $orderInfo['zjLid']?>'});" class="ui-link">查看战绩</a>
                                <?php endif; ?>
                                <?php else: ?>
                                <a href="javascript:;" onclick="location.href = '<?php echo $this->config->item('base_url')?>hemai/userinfo/<?php echo $infostrCode?>'" class="name"><?php echo uname_cut($orderInfo['uname'])?></a>
                                <div class="level"><?php echo calGrade($orderInfo['points'], 4)?></div>
                                <a href="javascript:;" onclick="location.href = '<?php echo $this->config->item('base_url')?>hemai/userinfo/<?php echo $infostrCode?>'" class="ui-link">查看战绩</a>
                                <?php endif; ?>
                            </td>
                        </tr>
                        <tr><th>认购人次</th><td><?php echo $orderInfo['popularity']?>人次，共<?php echo number_format(ParseUnit($orderInfo['buyTotalMoney'], 1))?>元<a href="javascript:;" onclick="window.webkit.messageHandlers.subscriptionList.postMessage({orderId:'<?php echo $orderInfo['orderId']?>'});" class="ui-link">认购列表</a></td></tr>
                        <tr><th>方案状态</th><td><?php echo parse_hemai_status($orderInfo['status'], $orderInfo['my_status'])?></td></tr>
                        <tr><th>盈利佣金</th><td><?php echo $orderInfo['commissionRate']?>%<?php if ($orderInfo['commission']) {echo "（".number_format(ParseUnit($orderInfo['commission'], 1), 2)."元）";}?><i class="ui-icon-tips tip0">?</i><a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/help/hemai#t-4';" class="ui-link">中奖怎么分</a></td></tr>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <?php }elseif (in_array($orderInfo['lid'], array(BetCnName::JCZQ, BetCnName::JCLQ))) {?>
                                    	<b><?php echo $passWay;?></b><b><?php echo $orderInfo['betNum']; ?>注</b><?php if ($orderInfo['playType'] < 7) echo "<b>".$orderInfo['multi']."倍</b>"; ?><b>共<?php echo ParseUnit( $orderInfo['money'], 1 ); ?>元</b>
                ...
                <?php }?>
                    ...
                    <div class="cp-box-bd">
                    	<table class="table-bet">
		                    ...
		                    <thead><tr><th>场次</th><th><?php if($orderInfo['lid'] == BetCnName::JCZQ){ echo '主队VS客队';}else { echo '客队VS主队';}?></th><th>玩法</th><th>投注方案<i>参考SP值</i></th></tr></thead><tbody><?php echo $award['tpl'];?></tbody>
		                </table>
                    </div>
            		<?php }elseif (in_array($orderInfo['lid'], array(BetCnName::SFC, BetCnName::RJ))) {?>
            		<div class="cp-box-bd">
                    ...
                    <?php if (in_array($orderInfo['lid'], array(BetCnName::SFC, BetCnName::RJ)) && $award['atpl']) {?><div class="pd30"><table class="table-info"><tbody><tr><th>开奖号码</th><td><b class="main-color"><?php echo $award['atpl'];?></b></td></tr></tbody></table></div><?php }?>
	                    ...
	                    <?php if(in_array($orderInfo['lid'], array(BetCnName::JCZQ, BetCnName::JCLQ)) && $orderInfo['playType'] == 7):?><a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/bonusopt/hm<?php echo $orderInfo['orderId']; ?>/<?php echo $strCode; ?>';">奖金优化明细</a><?php endif; ?>
	                    <?php if($orderInfo['status'] >= $orderStatus['draw'] && !in_array($orderInfo['status'], array(600, 610, 620))){?>
                        <a onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/tickets/hm<?php echo $orderInfo['orderId']; ?>/<?php echo $strCode; ?>'; " href="javascript:void(0);">查看出票明细</a>
                        <!-- 大乐透乐善码 start -->
                        <?php if(!empty($showdetail) && !empty($lsDetail['detail'])): ?>
                        <a href="<?php echo $this->config->item('pages_url'); ?>ios/hemai/lsDetail/<?php echo $orderInfo['orderId']; ?>" class="link-lsm">查看订单乐善码<?php if($lsDetail['totalMargin'] > 0):?>（中奖）<?php endif;?></a>
                        <?php endif;?>
                        <!-- 大乐透乐善码 end -->
                        <?php }?>
	                    </div>
    ...
    require(['basic', 'ui/loading/src/loading', 'ui/tips/src/tips'], function(basic, loading, tips){
        ...
        try {
            ...
            window.webkit.messageHandlers.getCoPurDetailInfo.postMessage({orderState:orderState, remain:'<?php echo empty($uid) ? '-1' : number_format(ParseUnit($uinfo['money'], 1), 2)?>', maxBuy:<?php echo ($orderInfo['money']-$orderInfo['buyTotalMoney'])/100?>,
            	isLaunter:<?php echo (int)($orderInfo['uid'] == $uid)?>, orderId:'<?php echo $orderInfo['orderId']?>', uname:'<?php echo $orderInfo['uname']?>', lid:<?php echo $orderInfo['lid']?>, cancelState:cancelState});
        }catch (e) {
        };
        ...
        $('.jcMatchLive').on('click', function(){
            ...
            if(jcMid > 0){
                try{
                    window.webkit.messageHandlers.gotoLive.postMessage({jcMid:jcMid,lid:lid});
                }catch(e){
                    // ..
                }
            }
        ...
        $("#gendan").on('click', function(){
           $.post("/ios/api/v1/gendan/hasGendan",
           {data:'<?php echo $postData;?>'},
            function(res){
                if(res.status==1){
                   window.webkit.messageHandlers.goCustomizationOrder.postMessage({uid:'<?php echo $orderInfo['uid'];?>',uname:'<?php echo $orderInfo['uname'];?>',lid:'<?php echo $orderInfo['lid'];?>',lidName:'<?php echo BetCnName::getCnName($orderInfo['lid'])?>',point:'<?php echo $orderInfo['points'];?>'});
                }
                if(res.status==300){
