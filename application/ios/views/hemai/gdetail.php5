...
<body>
    ...
    try {
        ...
        <?php if ($orderInfo['status'] > 1) { ?>
            gendan = '1';
        <?php } ?>
        window.webkit.messageHandlers.continueGendan.postMessage({uid:'<?php echo $orderInfo['puid'];?>',uname:'<?php echo $uname;?>',lid:'<?php echo $orderInfo['lid'];?>',lidName:'<?php echo BetCnName::getCnName($orderInfo['lid'])?>',point:'<?php echo $points;?>',hasGendan:gendan});
    }catch (e) {
