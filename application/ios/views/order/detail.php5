...
<body>
    <div class="wrapper bet-detail bet-detail-<?php echo $enName;?> <?php if (in_array($enName, array('jlks', 'jxks'))) {?>bet-detail-ks<?php } ?>">
        <!-- 延期公告 -->
        ...
        <section class="o2o-bet-detail">
            ...
            <p>出票部分成功</p>
            <p><a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/tickets/<?php echo $orderId; ?>/<?php echo $strCode; ?>';" ><em class="sub-color">查看出票明细</em></a><small>(未及时出票的投注金现已退至账户余额)</small></p>
            <?php elseif($order['status'] >= $orderStatus['draw']): ?>
            <p>已成功出票。</p>
            <!-- 大乐透乐善码 start -->
            <?php if(!empty($lsDetail['detail'])): ?>
            <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/lsDetail/<?php echo $orderId; ?>/<?php echo $strCode; ?>';" class="link-lsm">查看订单乐善码<?php if($lsDetail['totalMargin'] > 0):?>（中奖）<?php endif;?></a>
            <?php endif;?>
            <!-- 大乐透乐善码 end -->
            <?php endif;?>
        </section>
            ...
            <div class="lottery-info">
                ...
                <?php else:?>
                	<em>
                        ...
                        <?php if($order['status'] == $orderStatus['drawing']):?>
                            <a href="javascript:void(0);" onClick="window.location.href='/ios/order/ticketTime'; " class='lottery-info-timetips'></a>
                        <?php endif; ?>
                	</em>
                <?php endif;?>
                </p> 
        ...
        <?php elseif ($order['lid'] == BetCnName::SFC || $order['lid'] == BetCnName::RJ):?>
            ...
            <div class="cp-box-ft">
                ...
                <div class="bet-detail-lnk">
                    ...
                    <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/tickets/<?php echo $orderId; ?>/<?php echo $strCode; ?>'">查看出票明细</a> 
                    <?php endif; ?>
                </div>
            </div>
        ...
        <?php endif;?>
            ...
            <div class="cp-box-ft">
                <ul class="cp-list">
                    ...
                    <li>
                        <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/bonusopt/<?php echo $orderId; ?>/<?php echo $strCode; ?>';" class="ticket-detail">
                            <em>奖金优化</em>
                    ...
                    <?php if($order['status'] >= $orderStatus['draw'] && $order['status'] != $orderStatus['concel']): ?>
                    <li>
                        <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/tickets/<?php echo $orderId; ?>/<?php echo $strCode; ?>';" class="ticket-detail">
                            <em>出票明细</em>
        ...
        <div class="bet-detail-lnk"> 
            <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/order/winTips'; ">中奖后如何领奖？</a>     
            <?php if(!in_array($order['lid'], array(BetCnName::JCZQ, BetCnName::JCLQ, BetCnName::SFC, BetCnName::RJ, BetCnName::GYJ, BetCnName::GJ))):?>
            <!-- 数字彩展示 -->
            <a href="javascript:void(0);" onClick="window.location.href='<?php echo $this->config->item('pages_url'); ?>ios/help/play/<?php echo $enName; ?>'; ">中奖怎么算？</a>
            <?php endif; ?>    
        </div>   
        <?php if(!empty($banner)): ?>
    ...
    <script>
        ...
        require(['basic', 'ui/loading/src/loading', 'ui/tips/src/tips'], function(basic, loading, tips){
            ...
            $(function(){
                ...
                var buyPlatform = '<?php echo $order['buyPlatform']; ?>', codes = '<?php echo $orderPlan['codes']; ?>', lid = '<?php echo $orderPlan['lid']; ?>', betLid = '<?php echo $order['lid']; ?>', isChase = '<?php echo $orderPlan['isChase']; ?>', nolexuan = true;
                ...
                if(orderStatus == '10'){
                    ...
                    try{                    
                        ...
                        // 原生继续购买按钮
                        if (lid != '' && ($.inArray(lid, ['33', '52']) === -1 || buyPlatform != '0' || appVersionCode >= '11') && nolexuan) window.webkit.messageHandlers.continueBuy.postMessage({codes:codes,lid:lid,isChase:isChase});
                    }catch(e){
                        // ..
                    }
                }
            ...
            var appAction = "<?php echo $banner['appAction']; ?>", tlid = "<?php echo $banner['tlid']; ?>", 
            enName = "<?php echo $banner['enName']; ?>", webUrl = "<?php echo $banner['webUrl']; ?>";

            $('.img-link').on('click', function(){
                if(appAction == 'bet'){
                	window.webkit.messageHandlers.doBet.postMessage({lid:tlid});
