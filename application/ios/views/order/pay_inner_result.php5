    ...
    <div class="wrapper pay-result ios">
        <div class="pay-result-hd mod-result<?php if($pay_status != 'true'){echo '-false';}?>">
            ...
            <div class="mod-result-ft">
                ...
                ?>
                <?php if($versionInfo['appVersionCode'] < 3030001){ ?>
                <div class="btn-group"> 
                    <!-- 标准版 -->
                    <?php if($pay_status == 'true'):?>
                    <button class="btn btn-cancel" onClick="window.location.href='<?php echo $orderUrl; ?>';">查看订单详情</button>
                    <?php else: ?>
                        <button class="btn btn-cancel" onClick="window.webkit.messageHandlers.goUser.postMessage({});">查看账户详情</button>
                    <?php endif; ?>
                    <?php if ($orderType == 4 && $ctype == 1): ?>
                        <!-- 合买方案 -->
                        <button class="btn btn-confirm" onClick="window.webkit.messageHandlers.goCoPurHall.postMessage({lid:'<?php echo $lid; ?>'});">再来一单</button>     
                    ...
                    <?php else: ?>
                        <!-- 自购方案 -->
                        <button class="btn btn-confirm" onClick="window.webkit.messageHandlers.doBet.postMessage({lid:'<?php echo $lid; ?>'});">再来一单</button>
                    <?php endif; ?>
                </div>
                <?php }else{ ?>
                <div class="btn-group"> 
                ...
                <div class="btn-group"> 
                    <!-- 标准版 -->
                    <?php if($pay_status == 'true'):?>
                        <button class="btn btn-cancel" onClick="window.webkit.messageHandlers.goWeb.postMessage({'url':'<?php echo ($orderType==5?"https:":"").$orderUrl; ?>',popToRoot:1});">查看订单详情</button>
                    <?php else: ?>
                        <button class="btn btn-cancel" onClick="window.webkit.messageHandlers.goUser.postMessage({});">查看账户详情</button>
                    <?php endif; ?>
                    <button class="btn btn-cancel" onclick="window.webkit.messageHandlers.goMainActivity.postMessage({'tab':'tabHome'});">返回首页</button>
                </div>
                <?php } ?>
            </div>
        </div>    
    ...
    <script>
        ...
        require(['basic', 'ui/loading/src/loading', 'ui/tips/src/tips'], function(basic, loading, tips){
            ...
            $('.img-link').on('click', function(){
                if(appAction == 'bet'){
                    window.webkit.messageHandlers.doBet.postMessage({lid:'<?php echo $banner['tlid'];?>'});
                }else if(appAction == 'email'){
                    window.webkit.messageHandlers.bindEmail.postMessage({});
