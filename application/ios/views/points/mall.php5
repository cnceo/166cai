...
<body>
    <div class="wrapper p-jifen" v-cloak>
        ...
        <div class="user-pannel">
            ...
            <h2 class="user-jifen">
                <a href="/ios/points/pointList"><em>{{ user.grade_value }}</em>积分</a>
            </h2>
        </div>
        <div class="jifen-tab">
            ...
            <div class="jifen-tab-bd">
                ...
                <ul class="jfdh" v-if="tab.curIdx === 1 && redpacklist.length">
                    <li v-for="item in redpacklist">
                        <a :href="'/ios/points/redpackDetail/'+item.rid" class="jfdh-item" :class="{'disabled': item.out === 0}">
    ...
    <script>
        new Vue({
            ...
            created: function () {
                this.$http.get('/ios/points/getMemberInfo').then(function (res) {
            ...
            ready: function () {
                this.$http.get('/ios/points/getTaskList').then(function (res) {
                    var res = JSON.parse(res.data)
                    if (res.status === '200') {
            ...
            methods: {
                getPoint: function (id, index) {
                    this.$http.post('/ios/points/getPoint', {
                        'id': id
                    }, {
                        emulateJSON: true
                    }).then(function (res) {
                ...
                todo: function (id) {
                    switch(id){
                        case '1':
                        	window.webkit.messageHandlers.goMainActivity.postMessage({tab:'tabHome'});
                            break;
                        case '2':
                        ...
                        case '4':
                            ...
                            break;
                        case '5':
                        	window.webkit.messageHandlers.goMainActivity.postMessage({tab:'tabUnite'});
                            break;
                    }   
                },
                goMember: function(){
                	window.webkit.messageHandlers.toActivity.postMessage({action:'vipship'}); 
                }
            }
        })
    </script>
