...
<div class="wrap web" id="app" v-cloak>
      <div class="web-box">
          ...
          <div class="join-form">
            ...
            <div class="join-form-item yzm" v-if="!join">
              <input id="imgcode" @focus="push" @blur="pull" v-model="ajaxData.imgCode" detect-change="off" v-validate:imgcode="{required: {rule: true, message: '验证码不能为空'}, minlength: {rule: 4, message: '请输入4位验证码'}}" type="tel" placeholder="输入4位验证码" maxlength="4" class="input-yzm" initial="off">
				<img :src="'/ios/activity/captcha?v=' + random" @click="refresh" alt="">
        ...
        <template v-if="resData.status == 200">
          ...
          <a href="javascript:;" @click="dlApp(['_trackEvent', 'iosrequest', 'download', 'receive'])" class="btn-click-w">马上下载客户端使用</a>
        </template>
        ...
        <template v-if="resData.status == 300">
          ...
          <a href="javascript:;" @click="dlApp(['_trackEvent', 'iosrequest', 'download', 'fail'])" class="btn-click-w">马上下载客户端使用</a>
        </template>
    ...
    var newUser = new Vue({
      ...
      data: function () {
        return {
          ...
          api: {
            register: '/ios/activity/invitationDoRegister'
          },
      ...
      methods: {
        ...
        postEvent: function () {
          _hmt.push(['_trackEvent', 'iosclick', 'ioscent']);
          var _this = this;
