...
//        }.bind(this), 2000)
    ...
    methods: {
        shareFn: function () {
            _hmt.push(['_trackEvent', 'iosrequest', 'iosinvite']);
            if (!this.uid) {
            ...
            window.webkit.messageHandlers.snsShare.postMessage({url:this.url,title:"世界杯来了！推荐个中奖福地给你",content:"<?php echo $self['uname']?>邀你加入166彩票，一起玩赚世界杯！猛戳领取166元新人豪礼>>",imageUrl:"<?php echo $imgurl?>?0614"});
        },
        openQrCode: function () {
            if (!this.uid) {
                var backUrl = window.location.href;
