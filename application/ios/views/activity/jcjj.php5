...
<head>
    ...
    <title>竞彩2串1加奖</title>
    <link href="<?php echo getStaticFile('/caipiaoimg/static/css/active/plus-awards5.min.css');?>" rel="stylesheet" type="text/css" />
</head>
...
<body ontouchstart="">
    ...
    <script id="eurojj" type="text/html">
        <a href="javascript:;" target="_blank" class="plus-awards-hd">
            <img src="<?php echo getStaticFile('/caipiaoimg/static/images/active/plus-awards5/{{dataInfo.header.bannerName}}');?>" alt="">
            <h1>{{dataInfo.header.title}}</h1>
            <p class="plus-awards-time">活动时间：{{dataInfo.header.time[indexTime]}}</p>
        </a>
        <div class="plus-awards-bd">
            ...
            <div class="btn-group">
                <a href="javascript:;" onclick="window.webkit.messageHandlers.doBet.postMessage({lid:'42'});" target="_blank" class="btn btn-special btn-join">投注竞彩足球</a>
                <a href="javascript:;" onclick="window.webkit.messageHandlers.doBet.postMessage({lid:'43'});" target="_blank" class="btn btn-special btn-join">投注竞彩篮球</a>
            </div>
        </div>
    </script>
    ...
    <script>
	    $(function(){
			...
			$.get('/ajax/getTime', function(data){
				...
				$.get('/source/js/zmjj.js?20180504', function() {
	        		...
	        		$(arrTime).each(function (i, item) {
	            		...
	            		if (teamData.nowTime <= +new Date('2017/' + itemTime[1])) {
