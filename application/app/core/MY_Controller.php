...
class MY_Controller extends CI_Controller 
{
    ...
    public function checkUserAgent()
    {
        ...
        if( strpos($agentInfo, '2345caipiao/android') == FALSE && ENVIRONMENT === 'production')
        {
            echo "非法的浏览器请求！";die;
        }
    }
    ...
    public function getRequestHeaders()
    {
        ...
        $headerInfo = array(
            ...
            'deviceId' => (isset($headers['deviceId']) && !empty($headers['deviceId']))?$headers['deviceId']:'',
            // 手机型号
     ...
     * @return [type]        [description]
     */
    public function sendVoiceCodeMsg($phone, $ctype='captcha') 
    {
        $this->primarysession->startSession();
        if (!empty($phone)) 
        {
            $code = $this->_sendVoiceCodeMsg($phone,$ctype);
            if (!empty($code)) 
            {
                $out_time = $this->config->item('OUTTIME');
                $time = time();
                $expire = $time + 60 * $out_time['captche'];
                $second = $time + 60;
                if(!isset($code['Mobile'])) return false;
                $codestr = "{$code['VerifyCode']}:$expire:$second:{$code['Mobile']}:{$code['OrderNo']}";
                $this->primarysession->setArg($ctype, $codestr);
                return true;
            }
        }
    ...
    protected function _sendVoiceCodeMsg($phone, $sname='captcha') 
    {
        $this->primarysession->startSession();
        $code = array();
        $codestr = $this->primarysession->getArg($sname);
        if (!empty($codestr)) $codestr = explode(':', $codestr);
        $codestr_len = count($codestr); 
        if (($codestr[2] < time() && $codestr_len == 5) || $codestr_len == 4 ) 
        {
            $this->load->library('tools');
            $response = $this->tools->sendVoiceCodeMsg($this->uid,$phone);
            if (!empty($response)) 
            {
                $code = array(
    ...
    {
        $uinfo['money'] = ParseUnit($uinfo['money'], 1);
        ...
        $replyId = $this->info_comments_model->getReplyId($uinfo['uid']);
        $uinfo['replyId'] = $replyId ? (string)$replyId : '0';
        // 合买宣言
        $this->load->model('united_planner_model');
        $desInfo = $this->united_planner_model->getUserDescription($uinfo['uid']);
        $uinfo['united_intro'] = (!empty($desInfo['introduction']) && $desInfo['introduction_status'] == 1) ? $desInfo['introduction'] : $this->config->item('united_intro');
...
//        }
    ...
    public function checkUserLogin($uid)
    {
        ...
        $headerInfo = $this->getRequestHeaders();
        if(!empty($uid) && !empty($headerInfo['deviceId']) && $headerInfo['appVersionCode'] >= '40200')
        {
            $params = array(
                'uid'       =>  $uid,
                'idfa'      =>  $headerInfo['deviceId'],
