...
class JcBonusBuyContent
{
	...
	private function renderMatch($matchId, $cast, $matchInfo, $ticketInfo, $versionInfo) 
	{
		...
		$tpl .= ($versionInfo['appVersionCode'] >= 19) ? '<a href="javascript:;">' : '';
		$tpl .= $matchInfo['home'];
		$tpl .= '<b>' . $score . '</b>';
		$tpl .= $matchInfo['away'];
		$tpl .= ($versionInfo['appVersionCode'] >= 19) ? '</a>' : '';
		$tpl .= '</td>';
		$pts = array_keys($cast);
		$playCount = 0;
	...
	private function parseTicket($tickets, $ticketInfo = array())
	{
		$cast = array();
		if (!$tickets) 
		{
	...
	// 奖金优化主函数
	public function bonusOpt($cast, $matchInfo, $ticketData)
	{
		$matches = array();
		$matchInfo = $this->parseMatchInfo($matchInfo);
		$result = $this->parseBonusTicket($cast, $matchInfo, $ticketData);
		return $result;
	}

	// 解析场次
	private function parseMatchInfo($matches)
	{
		$matchInfo = array();
		if(!empty($matches))
		{
	...
	// 奖金优化 - 拆分投注串
	private function parseBonusTicket($tickets, $matchInfo, $ticketData)
	{
		$cast = array();
		if (!$tickets) 
		{
		...
		// 场次
		$ticketArr = $this->parseTicket($tickets, $ticketData);
		...
		foreach ($tickets as $ticket)
		{
			...
			// 场次
			$ticketInfo['matchInfo'] = '';
			// 过关方式
			$ticketInfo['type'] = $this->getType($parts[3]);
			// 倍数
			$ticketInfo['multis'] = $parts[2];

			$codesArr = explode(',', $codes);
			// 解析场次信息
			$tpl = '';
			$counts = count($codesArr);
			foreach ($codesArr as $key => $v)
			{
				$detail = explode('=', $v);
				if(strpos($v, '>') === false)
				{
				...
				$betType = '';
				$rule = '/([\d\-\:]*)(?:\{(.*)\})?\(?(\d*\.?\d*)?\)?/is';
				preg_match($rule,$detail[1],$matchs);
				if($matchs)
				{
					$betType = $matchs[1];
				}
				// 主队
				$tpl .= $matchInfo[$matchId]['home'];
				// 让球
				if($playType == 'RQSPF' && !empty($ticketArr[$matchId]['let']) && $ticketArr[$matchId]['let'] != '-')
				{
					// $let = intval($ticketArr[$matchId]['let']) > 0 ? '+' . intval($ticketArr[$matchId]['let']) : intval($ticketArr[$matchId]['let']);
					$tpl .= '(' . $ticketArr[$matchId]['let'] . ')';
				}elseif ($playType == 'RFSF' && !empty($ticketArr[$matchId]['let']) && $ticketArr[$matchId]['let'] != '-') {
					$tpl .= '(' . $ticketArr[$matchId]['let'] . ')';
				}
				else
				{
				...
				// 投注信息
				$tpl .= $this->_getCastName($playType, $betType);
				if($counts != ++$key)
				{
					$tpl .= '<br/>';
				}			
			}
			$ticketInfo['matchInfo'] .= $tpl;
			$ticketInfo['bonus'] = 0;
			$ticketInfo['status'] = '';
			array_push($cast, $ticketInfo);
		}
		return $cast;
	}
