...
if (!function_exists('getallheaders')) 
{
    function getallheaders() 
    {
        ...
        $headers['deviceId'] = (isset($headers['Deviceid']) && !empty($headers['Deviceid']))?$headers['Deviceid']:'';
        // 手机型号
...
{
    function ParseMoney($money = 0) 
    {
        // 取整
        $poolArry = explode('.', $money);
        $money = $poolArry[0];
        ...
        {
            $temp = str_split(strrev(floatval($money)), 4);
            // 升序
            krsort($temp);
            if(isset($temp[2]))
            {
...
if (!function_exists('ParseUrl'))
{
    function ParseUrl($content = '') 
    {
    	...
    	return preg_replace('/https\:\/\/888\.166cai\.cn\/info\/\w+\/(\d+)/', $info['data']['android'] . "/app/info/detail/$1", $content);
    }
}
