...
class Chase_Order_Model extends MY_Model
{
	...
	public function getChaseInfoById($params)
	{
		...
		$sql = "SELECT lid, status, chaseId, created, pay_time, money, setStatus, setMoney, endTime, chaseType,
				status, totalIssue, chaseIssue, revokeIssue, failIssue, codes, isChase, buyPlatform, bonus, is_hide, created 
	...
	public function getChases($cons, $cpage, $psize)
	{
		...
		$sql = "select m.chaseId, m.lid, m.created, m.totalIssue, m.chaseIssue, m.money, m.status, m.bonus, SUM(IF(o.`status` = '0', 1, 0)) as hasstop, m.endTime, m.is_hide
				from " . $table . " as m
	...
	public function hideChaseOrder($uid, $chaseId)
	{
		$sql = "UPDATE cp_chase_manage SET is_hide = (is_hide | 1) WHERE uid = ? AND chaseId = ? AND status > {$this->status['is_chase']}";
        return $this->db->query($sql, array($uid, $chaseId));
	}
}
