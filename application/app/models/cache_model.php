...
class Cache_Model extends MY_Model 
{
     ...
     * @date:2016-08-08
     */
    public function getSmartIssue($lotteryId)
    {
        $lotteryInfo = $this->lidChaseMap;
        $REDIS = $this->config->item('REDIS');
        ...
        {
            // 获取近两天的期次信息
            $maxTime = date('Y-m-d', strtotime('+ 2 days')) . ' 00:00:00';

            foreach ($followIssues as $key => $items) 
            {
                if($items['show_end_time'] < $maxTime)
                {
                    array_push($issueData, $items);
                }
            }
        }
        return $issueData;
    ...
    // 老版缓存兼容
    public function getBannerInfo($ctype, $platformId = 1, $platformName = 'android')
    {
        $ctypeArr = array(
