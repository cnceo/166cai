...
{   
    ...
    public function getOrder( $cons )
    {
        ...
        $sql = "SELECT o.orderId, o.codes, o.codecc, o.lid, o.money, o.multi, o.issue, o.playType, o.isChase, o.orderType, o.betTnum, o.status, o.my_status, o.bonus, o.margin, o.shopId, o.failMoney, o.activity_ids, o.activity_status, o.pay_time, o.created,o.buyPlatform,o.is_hide,o.forecastBonus,d.redpackId,d.redpackMoney 
    ...
    */
    ...
    public function getMyBetListByLid($con)
    {
        $sql = "SELECT orderId, money, issue, lid, codes, status, bonus, margin, my_status, created FROM cp_orders WHERE uid = ? AND lid = ? AND status >= {$this->status['pay']} AND created >= ? AND (is_hide & 1) = 0 ORDER BY created DESC LIMIT {$con['number']}";
        return $this->slave->query($sql, array($con['uid'], $con['lid'], $con['created']))->getAll();
    }

    public function countWinOrder($uid, $lid, $issue)
    {
        $sql = "SELECT count(*) FROM cp_orders WHERE uid = ? AND lid = ? AND issue = ? AND status = {$this->status['win']} AND sendprize_time > '0000-00-00 00:00:00'";
        return $this->slave->query($sql, array($uid, $lid, $issue))->getOne();
    }

    // 隐藏自购订单 - 客户端只有三个月内订单
    public function hideOrder($uid, $orderId)
    {
        $sql = "UPDATE cp_orders SET is_hide = (is_hide | 1) WHERE uid = ? AND orderId = ? AND status > {$this->status['draw']}";
        return $this->db->query($sql, array($uid, $orderId));
    }
