...
class Award_Model extends MY_Model {
    ...
    */
    ...
    {   
        ...
        else
        {
            ...
            elseif(in_array($lotteryId, array('21406', '21407', '21408', '54')))
            {
    ...
    */
     ...
     */
    public function getAwardsByIssue($lotteryId, $issues = array())
    {
    	$data = array();
    	$enName = $this->getTBName($lotteryId);
    	if($enName)
    	{
    		$table = "cp_" . $enName . "_paiqi";
    		$result = $this->slaveCfg->query("select issue, awardNum from $table where issue in ?",array($issues))->getAll();
    		foreach ($result as $val)
    		{
    			$data[$val['issue']] = $val['awardNum'];
    		}
