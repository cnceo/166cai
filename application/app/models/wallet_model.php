...
class Wallet_Model extends MY_Model 
{
    ...
    {
        $sql = "SELECT created, trade_no, orderId, subscribeId, additions, status, money, content
            FROM {$this->tbname}
    ...
    public function getChanelById($channelId)
    {
        $sql = "select name from cp_channel where id=? limit 1";
        return $this->db->query($sql, array($channelId))->getRow();
    }
    
    /*
     * APP 查询支付流水
     */
    public function getPayLogStatus($trade_no)
    {
        $sql = "select trade_no, status from cp_pay_logs where trade_no = ?";
        return $this->db->query($sql, array($trade_no))->getRow();
    }

    public function getPayConfigData($platform, $pay_type)
	{
		$sql = "SELECT id, platform, ctype, pay_type, mer_id, rate, weight, status FROM cp_pay_config WHERE platform = ? AND pay_type = ? AND status = 0 AND rate > 0 LIMIT 1";
		return $this->slave->query($sql, array($platform, $pay_type))->getOne();
	}
