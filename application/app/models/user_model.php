...
class User_Model extends MY_Model
{
    ...
    public function getCommentStatus($uid)
    {
        $sql="select uncomment from cp_user_info where uid = ?";
     ...
     * @return unknown
     */
    public function saveApps($data) {
        $sql = "insert into cp_app_installapp_logs (uid,platform,apps,created) values(?,?,?, now())
         on duplicate key update apps = values(apps)";
        return $this->db->query($sql, array($data['uid'], $data['platform'], $data['apps']));
    }

    public function editIntroduction($uid, $description)
    {
        $sql = "UPDATE cp_user_info SET introduction = ?, sensitive_words = '', introduction_status = 0, introduction_time = NOW() WHERE uid = ?";
        return $this->db->query($sql, array($description, $uid));
    }
}
