...
class Api_Follow_Model extends MY_Model
{
    ...
    public function followRelation($params)
    {
        if($params['follow_status'])
        {
            $fields = array_keys($params);
            $sql = "INSERT cp_united_follow(" . implode(',', $fields) . ", created)values(" . 
            implode(',', array_map(array($this, 'maps'), $fields)) .  ", now())" . " on duplicate key update follow_status = values(follow_status);";
            return $this->db->query($sql, $params);
        }
        else
        {
            $sql = "UPDATE cp_united_follow SET follow_status = 0 WHERE puid = ? AND uid = ?";
            return $this->db->query($sql, array($params['puid'], $params['uid']));
        }
    }
}