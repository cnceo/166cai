    ...
    <script>
        new Vue({
            ...
            created: function () {
                ...
                this.$http.post('/app/points/getRedPackInfo', this.pramas, {
                    emulateJSON: true
                }).then(function (res) {
            ...
            methods: {
                ...
                reqExchange: function () {
                    this.$http.post('/app/points/exchangeRedPack', this.pramas,{
                    emulateJSON: true
                }).then(function (res) {
                        var res = JSON.parse(res.data)
                        if (res.status === '200') {
                            ...
                            this.confirm.cb = function () {
                                window.location.href="/app/redpack/index/<?php echo $token; ?>";
                            }
                        }else if (res.status === '300') {
                            android.relogin(location.href);
                            return;
