    ...
    <div class="wrapper recharge-detail pay-result">
        <div class="recharge-detail-hd mod-result<?php if(!$status){echo '-false';}?>">
            ...
            <div class="mod-result-ft">
                <div class="btn-group">
                   <a href="javascript:void(0)" onclick="<?php if($redirectPage == 'order'): ?>window.location.href='<?php echo $payView;?>'<?php else: ?>android.goIntentUser()<?php endif; ?>" class="btn btn-confirm">完成</a> 
                </div>
            </div>
        </div>
    </div>
