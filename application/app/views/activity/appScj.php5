...
<body ontouchstart="">
    <div class="wrap">
        <div class="wrap-content">
            <?php if($hasAttend == '1'): ?>
            <div class="hb-after">
                <a href="javascript:void(0);" onclick="android.showRedPacket();" class="btn-click" id="btn-click">查看红包</a>
                <h2>已领取过红包</h2>
            </div>
            <?php else: ?>
            <div class="hb" id="hb">
...
<script>
    ...
    require(['basic', 'ui/loading/src/loading', 'ui/tips/src/tips'], function(basic, loading, tips){
        ...
        $('.hb').on('tap', '.btn-click', function(){
            var $this = $(this);
            var Bcp = $(this).parents('.hb');
            if(Bcp.hasClass('hb-after')){
               android.showRedPacket();
            }else{
                if(closeTag){
                closeTag = false;
