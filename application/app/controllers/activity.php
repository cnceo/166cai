...
class Activity extends MY_Controller {
    ...
    // 加奖主力欧洲杯
    public function zlozb()
    {
        $this->load->view('activity/zlozb');
    }
    ...
    {    	
    	...
    	$data = array(
    			'users' => (empty($this->uid) && !$showBind) ? array() : $this->activity_model->getJoined($this->uid),
    			'chj' =>  (empty($this->uid) && !$showBind) ? array() : $this->activity_model->getchoujiang($this->uid),
    			'showBind' => $showBind,
    			'url' => $this->config->item('pages_url') . "activity/join/".$this->uid."/5",
    			'uid' => $this->uid,
    			'info' => $this->activity_model->getInfo()
    	);
    	$this->load->view('/activity/joinus', $data);
    }
    ...
    public function chj()
    {
        ...
        $postData = array(
            'uid' => $this->uid,
            'platformId' => 1,
            'channelId' => $this->recordChannel($versionInfo['channel']),
        );
        ...
        {
            // $postUrl = $this->config->item('pages_url');
            $this->config->item('protocol') . $this->config->item('pages_url');
        }

        $res = $this->tools->request($postUrl . 'api/activity/draw', $postData);
    ...
    // 贷款王新年活动
    public function appOutScjtoloan()
    {
        $this->load->view('/activity/dkw');
    }

    // 发送短信
    public function sendEventSms($ctype = 'dkw')
    {
        $data = $this->input->post(null, true);
		...
		if(!in_array($ctype, array('dkw', 'newUser')))
		{
		...
		$endTime = array(
			'dkw' => '2017-02-03 23:59:59',
			'newUser' => '2017-04-01 23:59:59',
		);
        // 活动时间
        if(date('Y-m-d H:i:s') > $endTime[$ctype])
        {
            $result = array(
                'status' => '0',
                'msg' => '活动已过截止时间',
        ...
        if ($this->primarysession->getArg($ctype . 'Captcha') != strtolower($data['imgCaptcha']))
        {
            $result = array(
                'status' => '0',
                'msg' => '验证码不正确',
        ...
        {
            // 清除
            $this->primarysession->setArg($ctype . 'Captcha', '');
            ...
            if($attendResult['status'] == '200')
            {
                ...
                $msg = $message['hongbao_' . $ctype];
                $res = $this->tools->sendSms(0, $data['phone'], $msg, 10, UCIP, 193);
                $res = json_decode($res, true);
    ...
    // 新注册领红包
    public function appOutNewUser()
    {
    	$this->load->view('/activity/newUser');
    }
    ...
    public function invitation()
    {
        ...
        $uid = $this->uid;
        $url = $this->config->item('protocol') . "//www.166cai.cn/app/activity/invitationRegister?userId=" . $self['id'] . "&cpk=" . $cpk;
        $imgurl = $this->config->item('protocol') . "//www.166cai.cn/caipiaoimg/static/images/app-icon-new.png";
        $this->load->view('/activity/invitation', compact('users', 'self', 'showBind', 'uid', 'url', 'imgurl', 'count', 'time'));
    }
    ...
    public function xnchj()
    {
        ...
        $res = $this->activity_xn_model->draw($this->uid, 1, 13);
    ...
    public function gpsend() {
        $this->load->view("/activity/gpsend");
    }
    ...
    public function worldcup2018($ename = '') {
        switch ($ename) {
            ...
            default:
                ...
                $max_pissue = $this->winrank->getNewPissue(3);
                $money = 0;
                if (time() > strtotime("2018-06-11")) {
                    $jc = $this->winrank->getJcMoney();
                    if (!empty($jc)) {
                        $money = $jc['money'] / 10;
                    }
                }
                ...
                if (time() > strtotime($max_pissue['start_time']) && time() < strtotime($max_pissue['end_time'])) {
                    $user = $this->winrank->getTopUser(3, $max_pissue['max_pissue']);
                ...
                if ($time >= strtotime("2018-05-14") && $this->uid) {
                    $hbstatus = $this->winrank->getHbGameStatus($this->uid);
                }
                if ($time >= strtotime("2018-06-11") && $this->uid) {
                    $dtstatus = $this->winrank->getDtGameStatus($this->uid);
                }
                if ($time >= strtotime("2018-07-16") && $this->uid) {
                    $jcstatus = $this->winrank->getJcGameStatus($this->uid);
                }
    ...
    public function getwcredpack() {
        ...
        $res = $this->libredpack->hongbaoworldcup('attend', array('uid' => $this->uid, 'phone' => $uinfo['phone'], 'platformId' => 1, 'channel' => $this->recordChannel($channel)));
        ...
        switch ($type) {
            ...
            default:
                ...
                if ($this->uid) {
                    ...
                    if ($this->uinfo['id_card']) $url = '/app/wallet/recharge/'.urlencode($this->strCode(json_encode(array('uid' => $this->uid, 'checked_idName' => 'payJd')), 'ENCODE'));
                }
