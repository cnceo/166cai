 ...
 class Notice extends MY_Controller 
{
	...
	public function getMsgType($msgType, $items)
	{
		...
		{
			// 网站公告
			$url = $this->config->item('pages_url') . 'app/notice/detail/' . $items['id'];
		}
		else
		{
			// 推广活动
	...
	public function message($token)
	{
		...
		$data = array(
			...
			'order_draw_f' => (!isset($uinfo['app_push']) || ($uinfo['app_push'] & 1) != 0) ? false : true, 
			'order_draw_s' => (isset($uinfo['app_push']) && ($uinfo['app_push'] & 2) != 0) ? false : true,
		...
		if(!empty($this->uid))
		{
			switch ($ctype)
			{
				...
				case 'email_send':
					...
					break;
				case 'order_draw_f':
					$result = $this->setPushStatus($this->uid, $status, 1);
					break;
				case 'order_draw_s':
					$result = $this->setPushStatus($this->uid, $status, 2);
					break;
				case 'win_prize':
	...
	public function setPushStatus($uid, $status, $position)
	{
		// 组装参数
		$postData = array(
			'uid' => $uid,
		...
		if($postStatus['codes'] == '200')
		{
			$result = array(
				'status' => '1',
				'msg' => '修改成功',
				'data' => $postData['push_status'] ? false : true
			);
		}
		else
		{
