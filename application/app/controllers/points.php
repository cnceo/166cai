...
class Points extends MY_Controller
{
    ...
    public function getPointLists()
    {
        ...
        foreach ($logs['res'] as $k => $log) {
            ...
            if ($log['ctype'] == 0) {
                if ($log['status'] == 0) {
                    $pointLists[$k]['url'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/order/detail/' . $log['orderId'] .'/'. $token;
                } elseif ($log['status'] == 1) {
                    $pointLists[$k]['url'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/order/detail/' . $log['orderId'] .'/'. $token;
                } elseif ($log['status'] == 2) {
                    $pointLists[$k]['url'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/hemai/detail/hm' . $log['orderId'] .'/'. $token;
                } else {
                    $pointLists[$k]['url'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/points/jfdetail/' . $log['trade_no'] .'/'. $token;
                }
            } else {
                $pointLists[$k]['url'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/points/jfdetail/' . $log['trade_no'] .'/'. $token;
            }
