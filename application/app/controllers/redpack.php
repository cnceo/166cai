	   ...
	   if($items['status'] == '1' && $items['p_type'] == '2')
		{
			if($items['valid_start'] <= date('Y-m-d H:i:s') && $items['valid_end'] >= date('Y-m-d H:i:s'))
			{
				...
				$data['url'] = $this->config->item('pages_url') . 'app/wallet/recharge/' . urlencode($token);
			}
		}
		else if($items['p_type'] == '3')
		{
			...
			{
				if($versionInfo['appVersionCode'] < '18')
				{
					$data['class'] = 'tipToken';
				}
				else
			...
			else
			{
				...
				else if($items['valid_start'] <= date('Y-m-d H:i:s') && $items['valid_end'] >= date('Y-m-d H:i:s'))
				{
