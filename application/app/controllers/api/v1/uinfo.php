...
class Uinfo extends MY_Controller 
{
	...
	public function sendRegSms()
	{
		...
		{
			// 短信登录
			if($checkStatus && $headerInfo['appVersionCode'] < '30800')
			{
				//手机号不存在提示  msg不能有值  app端要求 有值会出弹层
				$result = array(
					'status' => '-103',
					'needCaptcha' => '0',
					'msg' => '该手机号不存在', 
		...
		// 限制后图形验证码
		if($headerInfo['appVersionCode'] >= '8' && !empty($data['captcha']))
		{
			$captcha = isset($data['captcha']) ? $data['captcha'] : '';
			$sessionid = isset($data['sessionid']) ? $data['sessionid'] : '';
		...
		{
			$result = array(
				'status' => ($headerInfo['appVersionCode'] >= '8')?'-2':'0',
				'needCaptcha' => ($headerInfo['appVersionCode'] >= '8')?'1':'0',
				'msg' => '操作频繁，请稍后再试',
				'data' => array()
			);
			echo json_encode($result);
		...
		// 设备号限制
		if($headerInfo['appVersionCode'] >= '3' && !empty($headerInfo['deviceId']))
		{
		...
		// 防刷限制 end
		if(!isset($data['sendType']) || $data['sendType'] ==0 )
		{
			$sendStatus = $this->getSmsCode($data['phone'], $data['ctype'], 'yycaptcha');
			$suc_msg = '短信验证码发送成功';
			$err_msg = '短信验证码发送失败';
		}
		if( isset( $data['sendType'] ) && $data['sendType'] ==1 )
		{
			$sendStatus = $this->sendVoiceCodeMsg($data['phone'], 'yycaptcha');
			$suc_msg = '语音验证码发送成功';
			$err_msg = '语音验证码发送失败';
		}
		if($sendStatus)
	...
	public function sendSmsByWxlogin()
	{
		...
		*/
		...
		$res = $this->getSmsCode($data['phone'], 'register', 'yycaptcha');
	...
	public function wxRegister()
	{
		...
		*/
		...
		$userData = array(
			...
			'phone'		=>	$data['phone'],
			'password'	=> 	$this->createNonceStr(rand(6, 8)),
			'ip' 		=> 	UCIP,
			'platform'	=> 	$this->config->item('platform'),
			'version' 	=> 	isset($headerInfo['appVersionName']) ? $headerInfo['appVersionName'] : '1.0',
			'channel' 	=> 	$this->recordChannel($headerInfo['channel'])
	...
	public function smsRegister()
	{
		...
		*/
		...
		if($this->comm->abslength($data['password']) < 6 || $this->comm->abslength($data['password']) > 16)
		{
			$result = array(
				'status'		=>	'0',
				'msg' 			=>	'密码长度为6-16个字符',
