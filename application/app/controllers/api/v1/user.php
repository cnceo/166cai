...
class User extends MY_Controller 
{
	...
	public function userBaseInfo()
	{
		...
		if(!empty($data['uid']))
		{
			...
			{			
				...
				// 检查用户登录状态
				if(!$this->checkUserAuth($uinfo, $data['auth']) && $this->versionInfo['appVersionCode'] >= 2)
				{
					$result = array(
				...
				// 用户是否注销
				if(isset($uinfo['userStatus']) && $uinfo['userStatus'] == '1' && $this->versionInfo['appVersionCode'] >= '2')
				{
					$result = array(
						'status'	=>	($this->versionInfo['appVersionCode'] >= '40200') ? '700' : '300',
	...
	public function setBankInfo()
	{
		...
		// log_message('LOG', "绑定银行卡 - 请求参数: " . json_encode($data), 'setBankInfo');
	...
	/**
	 * [sendVoiceCode 发送语音短信]
	 * @author LiKangJian 2017-05-23
	...
	public function sendSmsCode()
	{
		...
		if($headerInfo['appVersionCode'] >= '8')
		{
			if(empty($data['uid']))
			{
				$result = array(
	...
	public function setPwordByPwd()
	{
		...
		if(empty($data['uid']) || empty($data['new_pword']) || empty($data['old_pword']) || empty($data['auth']))
		{
			$result = array(
				'status' => '0',
				'msg' => '缺少必要参数',
		...
		// 检查密码不能与支付密码一致
		// if(md5($data['new_pword']) == $info['pay_pwd'])
		// {
  //           $result = array(
  //               'status' => '0', 
  //               'msg' => '不能与支付密码一致',
  ...
  //       }
	...
	public function setNickName()
	{
		...
		if(!empty($data['uid']) && !empty($data['nick_name']))
		{
			$info = $this->user_model->getUserInfo($data['uid']);
    ...
    public function pushNotice()
    {
    	...
    	if($data['uid'] && $data['push_time'] && $data['switch'])
    	{
	    	...
	    	// 解析提交参数
	    	foreach (explode(',', $data['switch']) as $key => $items) 
	    	{
...
//        );
        ...
        foreach ($level as $j => $v) {
            ...
            foreach ($privilege as $v) {
                ...
                $privileges[$j][$k]['jumpurl'] = $this->config->item('protocol') . $this->config->item('pages_url') . 'app/points/privilege/' . $privileges[$j][$k]['id'];
                switch ($k) {
                    case 0:
...
//        );
    ...
    public function installedApps() {
        $data = $this->strCode($this->input->post('data'));
        $data = json_decode($data, true);
        if($data['uid'] && $data['installedApps']) {
            ...
            $this->user_model->saveApps($insertData);
        }
    }
}