...
class Trend extends MY_Controller 
{
	...
	public function dealTrendData($lid, $missData, $rows, $awardInfo)
	{
		...
		$awardNums = array();
		if(in_array($lid, array('53', '56', '57')))
		{
			$tmpData = array();
		...
		if(in_array($lid, array('53', '54', '56', '57')))
		{
	...
	public function ballFormat($detail, $lid, $type = 1)
	{
        ...
        else
        {
            foreach ($ballInfo as $playType => $items) 
            {
                ...
                if(in_array($lid, array('53', '56', '57')) && (!in_array($typeName, array('jiben', 'hz', 'hm_xingtai', 'kuadu'))))
                {
                	continue;
                }
    ...
    // 组合遗漏
    public function currentMiss()
    {
        $lid = $this->input->get('lid', TRUE);
        $playType = $this->input->get('playType', TRUE);
        $modType = $this->input->get('modType', TRUE);

        $this->load->model('miss_model','Miss');
        
        if(!in_array($lid, array(21406, 21407, 21408, 21421)) || empty($playType))
        {
            $result = array(
                'status'    =>  '0',
                'msg'       =>  '请求参数错误',
        ...
        $info = $this->Miss->getMissData($lid, $playType, $modType);
