...
class Order extends MY_Controller 
{
		...
		if( isset($data['uid']) && isset($data['orderId']) && !empty($data['uid']) && !empty($data['orderId']) )
		{
			...
			if ($data['orderType'] == '4') 
			{
				...
				if (!in_array($orderInfo['status'], array($orderStatus['create'], $orderStatus['pay'], $orderStatus['drawing'], $orderStatus['draw'])))
				{
					...
					header('Location: ' . $this->config->item('pages_url') . 'app/hemai/detail/hm'. $data['orderId'] . '/'. urlencode($codeStr));
				}
				...
				else 
				{
					...
					// 充值配置
                                $payconfig = $this->getPayConfig($this->getUserAgentInfo());
        			$orderInfo['payConfig'] = $payconfig;
				...
				if(!empty($order))
				{
					if( $order['info']['status'] == $orderStatus['create'] )
					{
						...
						if( $order['info']['money'] > $money['money'] )
						{
							...
							$orderData['appVersion'] = $this->getUserAgentInfo();
							// 充值配置
                                                        $payconfig = $this->getPayConfig($this->getUserAgentInfo());
        					$orderData['payConfig'] =  $payconfig;
        					$orderData['versionInfo'] = $this->getUserAgentInfo();
					...
					else
					{
						...
						// 已付款 跳转至详情页
						header('Location: ' . $this->config->item('pages_url') . 'app/chase/detail/'. $data['orderId'] . '/'. urlencode($codeStr));
					}
				}
			}
			else
			{
				...
				if(!empty($order))
				{
					...
					else
					{
						...
						// 已付款 跳转至详情页
						header('Location: ' . $this->config->item('pages_url') . 'app/order/detail/'. $data['orderId'] . '/'. urlencode($codeStr));
					}
				}
				else
				{
	...
	public function detail($orderId, $strCode)
	{
		...
		if (!empty($orderResponse['data']))
		{
			...
			else
			{
				...
				{
					// 是否包含组三单式
					if(strpos($order['codes'], ':2:1') !== FALSE && $versionInfo['appVersionCode'] < '11')
					{
						$order['codes'] = '';
					}
				}
	...
	*/
	...
	/*
 	 ...
 	 * @date:2015-11-25
 	 */
	public function bonusopt($orderId, $strCode = null)
	{	
		$data = $this->strCode(urldecode($strCode));
		$data = json_decode($data, true);
		$uid = isset($data['uid']) ? $data['uid'] : '';
    ...
    private function getDetailBanner($lid)
    {
    	...
    	$info = $this->Cache->getPreloadInfo($platform = 'android', 'orderdetail');
    	$detail = $info[$lid];
    ...
    public function orderDel()
    {
    	...
    	if(!empty($data['uid']) && !empty($data['orderId']) && $data['uid'] == $this->uid)
    	{
    		if($this->Order->hideOrder($data['uid'], $data['orderId']))
    		{
    			$result = array(
	                'status' => '1',
	                'msg' => '订单删除成功',
