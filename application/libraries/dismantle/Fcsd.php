...
class Fcsd
{
	...
	public function dismantle($order)
	{
    	...
    	$dis_results = $this->CI->libcomm->marge_arr($dis_results, $zx_results);
    	$bdata['s_data'] = array();
    	$bdata['d_data'] = array();
    	$count = 0;
    	...
    	{
    		$multis = $this->CI->libcomm->disMulti($betcbt['betnum'], $_multi, $order['lid']);
    		if(!empty($multis))
    		{
