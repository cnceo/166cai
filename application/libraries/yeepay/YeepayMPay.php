...
// 包含RSA、AES相关加解密包
if (!class_exists('Crypt_Rijndael')) include YEEPAY_PATH . '/Crypt_Rijndael.php';
if (!class_exists('Crypt_AES'))      include YEEPAY_PATH . '/Crypt_AES.php';
if (!class_exists('Crypt_DES'))		 include YEEPAY_PATH . '/Crypt_DES.php';
if (!class_exists('Crypt_Hash'))	 include YEEPAY_PATH . '/Crypt_Hash.php';
if (!class_exists('Crypt_RSA'))		 include YEEPAY_PATH . '/Crypt_RSA.php';
if (!class_exists('Crypt_TripleDES'))include YEEPAY_PATH . '/Crypt_TripleDES.php';
if (!class_exists('Math_BigInteger'))include YEEPAY_PATH . '/Math_BigInteger.php';
// 接口类型
...
class YeepayMPay 
{
	...
	public function init($initDatas){
		$this->account = $initDatas['merchantaccount'];
		$this->merchantPublicKey = $initDatas['merchantPublicKey'];
		$this->merchantPrivateKey = $initDatas['merchantPrivateKey'];
		$this->yeepayPublicKey = $initDatas['yeepayPublicKey'];
	 ...
	 * @return string
	 */
	public function webPay($datas)
	{
		$query = array(
			'orderid'	=>	$datas['order_id'],
			'transtime'	=>	$datas['transtime'],
			'currency'	=>	$datas['currency'],
			'amount'	=>	$datas['amount'],
			'cardno' =>	$datas['cardno'],
			'idcardtype' => $datas['idcardtype'],
			'idcard' => (string)$datas['idcard'],
			'owner' => $datas['owner'],
			'productcatalog'=>	$datas['product_catalog'],
			'productname'	=>	$datas['product_name'],
			'productdesc'	=>	$datas['product_desc'],
			'identityid'	=>	(string)$datas['identity_id'],
			'orderexpdate'	=>	$datas['orderexp_date'],
			'identitytype'	=>	$datas['identity_type'],
			'terminaltype' => $datas['terminaltype'],
			'terminalid' => (string)$datas['terminalid'],
			'userip'	=>	$datas['user_ip'],
			'userua'	=>	$datas['user_ua'],
			'callbackurl'	=>	$datas['callbackurl'],
			'fcallbackurl'	=>	$datas['fcallbackurl']
		);
