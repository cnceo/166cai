...
class JdPay extends RechargeAbstract
{
	/**
	 * 商户id
	 * @var unknown_type
	...
	public function requestHttp($params)
	{
		...
		$defaultParams['tradeTime'] = date('YmdHis', time());
		$defaultParams['tradeNum'] = $params['trade_no'];
		$defaultParams['amount'] =  (string) $params['money'];
		$defaultParams['ip'] = $params['ip'];
		$defaultParams['userId'] = $params['uid'];
		$defaultParams['specName'] = $params['real_name'];
		$defaultParams['specId'] = $params['id_card'];
		
		$returnData = array(
			'code' => true,
			'msg' => '请求成功',
			'data' => array('html' => $this->createForm($defaultParams)),
		);
	...
	public function notify()
	{
		...
		{
		    $message = '服务器IP：' . $_SERVER['SERVER_ADDR'] . '<br/>京东支付商户号：'. $this->merId . '异步通知接口验签失败，请及时留意。<br/>请求返回数据：' . $this->printJson($resdata);
	...
	public function syncCallback()
	{
		...
		{
		    $message = '服务器IP：' . $_SERVER['SERVER_ADDR'] . '<br/>京东支付商户号：'. $this->merId . '同步通知接口验签失败，请及时留意。<br/>请求返回数据：' . $this->printJson($_POST);
            $this->alertEmail($message);
            $this->log($param, __CLASS__, $this->merId, __FUNCTION__);
		}
	...
	private function createForm($params)
	{
		...
		$param["merchant"] = $params["merchant"];
		$param["device"] = $params["device"];
		$param["tradeNum"] = $params["tradeNum"];
		$param["tradeName"] = $params["tradeName"];
		$param["tradeDesc"] = $params["tradeDesc"];
		$param["tradeTime"] = $params["tradeTime"];
		$param["amount"] = $params["amount"];
		$param["currency"] = $params["currency"];
		$param["note"] = $params["note"];
		$param["callbackUrl"] = $params["callbackUrl"];
		$param["notifyUrl"] = $params["notifyUrl"];
		$param["ip"] = $params["ip"];
		$param["specCardNo"] = $params["specCardNo"];
		$param["specId"] = $params["specId"];
		$param["specName"] = $params["specName"];
		$param["userType"] = $params["userType"];
		$param["userId"] = $params["userId"];
		$param["expireTime"] = $params["expireTime"];
		$param["orderType"] = $params["orderType"];
		$param["industryCategoryCode"] = $params["industryCategoryCode"];
		$desKey = $this->desKey;
		$unSignKeyList = array ("sign");
		$sign = $this->signWithoutToHex($param, $unSignKeyList);
		$param["sign"] = $sign;
		$keys = base64_decode($desKey);
		$html =  '<body onLoad="document.autosubmit.submit();">';
		$html .= '<form name="autosubmit" action="'.$this->payGateway.'" method="post">';
		foreach ($param as $key => $value)
		{
			if(in_array($key, array('version', 'merchant', 'sign')))
			{
				$html .= '<input type="hidden" name="'.$key.'" value="'.$value.'"/><br/>';
			}
			elseif($value != null && $value != '')
			{
				$html .= '<input type="hidden" name="'.$key.'" value="'.$this->encrypt2HexStr($keys, $value).'"/><br/>';
			}
			else
			{
				$html .= '<input type="hidden" name="'.$key.'" value="'.$value.'"/><br/>';
	...
	{
		$pi_key =  openssl_pkey_get_private(file_get_contents(dirname(__FILE__) . '/jdpaykey/' . $this->merId . '/seller_rsa_private_key.pem'));//这个函数可用来判断私钥是否是可用的，可用返回资源id Resource id
		$encrypted="";
