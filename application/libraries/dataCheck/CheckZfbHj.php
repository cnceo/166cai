...
class CheckZfbHj
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_hjzfb_' . $name[2] . '.csv';
		//走充值验签获取对账信息
		$c_type = explode(',', $this->config['c_type']);
		$configId = $c_type[0];
		require_once APPPATH . '/libraries/recharge/HjPay.php';
		$extra = $this->CI->dataModel->getPayConfig($configId);
		$extra = json_decode($extra['extra'], true);
		$paySubmit = new HjPay($extra);
		$rparams = array(
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					$datas[] = array('trade_no' =>trim($data['5']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['10']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
