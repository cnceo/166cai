...
class CheckPaPay
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_papay_' . $name[2] . '.csv';
		//走充值验签获取对账信息
		$c_type = explode(',', $this->config['c_type']);
		$configId = $c_type[0];
		require_once APPPATH . '/libraries/recharge/PaPay.php';
		$extra = $this->CI->dataModel->getPayConfig($configId);
		$extra = json_decode($extra['extra'], true);
		$paySubmit = new PaPay($extra);
		$rparams = array(
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					if(trim($data['16']) == '外部订单号')
					{
						$spl_object->next();
						continue;
					}
					$datas[] = array('trade_no' =>trim($data['16']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['4']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
