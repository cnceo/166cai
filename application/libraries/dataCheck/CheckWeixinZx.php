...
class CheckWeixinZx
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_zxwx_' . $name[2] . '.csv';
		//走充值验签获取对账信息
		$c_type = explode(',', $this->config['c_type']);
		$configId = $c_type[0];
		require_once APPPATH . '/libraries/recharge/ZxWeixinPay.php';
		$extra = $this->CI->dataModel->getPayConfig($configId);
		$extra = json_decode($extra['extra'], true);
		$paySubmit = new ZxWeixinPay($extra);
		$rparams = array(
			'bill_date' => date('Ymd', $checkTime),
			'userPwd' => $this->config['extend'],
		);
		...
		if($respone['code'] && $respone['msg'] != '无数据')
		{
			//将第三方数据保存文件
			$locZip =  $this->logPath . date('Ymd', $checkTime) . '_zxwx_' . $name[2] . '.zip';
			file_put_contents($locZip, $respone['data']);
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					$datas[] = array('trade_no' =>trim($data['6']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['14']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
