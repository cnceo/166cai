...
class CheckWeixinWft
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_wftwx_' . $name[2] . '.csv';
		//微信按商户号拉取对账，同一个商户号不同渠道为了避免重复下载 故做判断
		if(!file_exists($locFile) || $r_flag)
		{
			//走充值验签获取对账信息
			$c_type = explode(',', $this->config['c_type']);
			$configId = $c_type[0];
			require_once APPPATH . '/libraries/recharge/WftPay.php';
			$extra = $this->CI->dataModel->getPayConfig($configId);
			$extra = json_decode($extra['extra'], true);
			$paySubmit = new WftPay($extra);
			$rparams = array(
					'bill_date' => date('Ymd', $checkTime),
					'bill_type' => 'SUCCESS',
			);
			$respone = $paySubmit->queryBill($rparams);
			$decodeRes = json_decode($respone, true);
			if(isset($decodeRes['status']))
			{
			...
			//将第三方数据保存文件
			file_put_contents($locFile, $respone);
			unset($respone);
		}
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					if(($data['11'] != '支付成功') || ($data['10'] != $this->config['extend']))
					{
						$spl_object->next();
						continue;
					}
					$datas[] = array('trade_no' =>trim($data['8']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['14']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
