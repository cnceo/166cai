...
class checkYeeMpay
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_yeempay_' . $name[2] . '.csv';
		//走充值验签获取对账信息
		$c_type = explode(',', $this->config['c_type']);
		$configId = $c_type[0];
		require_once APPPATH . '/libraries/recharge/YeepayMPay.php';
		$extra = $this->CI->dataModel->getPayConfig($configId);
		$extra = json_decode($extra['extra'], true);
		$paySubmit = new YeepayMPay($extra);
		$rparams = array(
			'bill_date' => date('Y-m-d', $checkTime),
		);
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					$datas[] = array('trade_no' =>trim($data['4']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['6']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
