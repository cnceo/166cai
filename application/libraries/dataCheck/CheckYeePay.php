...
class checkYeePay
{
	...
	private $ftpConfig = array(
			'10013415282' => array(
				...
				'username' => 'trx10013415282',
				'password' => '8fcf81e8',
				'port' => '21',
			),
			'10013356857' => array(
				'hostname' => 'reportftp.yeepay.com',
				'username' => 'trx10013356857',
				'password' => '19108af3',
				'port' => '21',
			),
	);
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_yeepay_' . $name['2'] . '.csv';
		//按商户号拉取对账，同一个商户号不同渠道为了避免重复下载 故做判断
		if(!file_exists($locFile) || $r_flag)
		{
			$remFile = date('Ymd', $checkTime) . '.csv';
			$this->CI->ftp->connect($this->ftpConfig[$name['2']]);
			$locZip =  $this->logPath . date('Ymd', $checkTime) . '_yeepay_' . $name[2] . '.zip';
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof()) 
				{
					...
					if(count($data) != '10' || $data['5'] != '166cp')
					{
						$spl_object->next();
						continue;
					}
					$datas[] = array('trade_no' =>trim($data['0']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['3']) * 100);
					$spl_object->next();
				}
				$start += $pSize;
