...
class checkUmPay
{
	...
	public function exec($config, $r_flag = 0)
	{
		...
		$locFile = $this->logPath . date('Ymd', $checkTime) . '_umpay_' . $name[2] . '.csv';
		//走充值验签获取对账信息
		$c_type = explode(',', $this->config['c_type']);
		$configId = $c_type[0];
		require_once APPPATH . '/libraries/recharge/UmPay.php';
		$extra = $this->CI->dataModel->getPayConfig($configId);
		$extra = json_decode($extra['extra'], true);
		$paySubmit = new UmPay($extra);
		$rparams = array(
		...
		if($spl_object)
		{
			...
			for($i = 1; $i <= $pages; $i++)
			{
				...
				while ($num-- && !$spl_object->eof())
				{
					...
					if(($data['0'] == 'TRADEDETAIL-START') && ($data['4'] != '00000'))
					{
						$this->CI->dataModel->updateCheckConfig($config['id'], array('fail_times' => $config['fail_times'] + 1));
						if(($config['fail_times'] + 1) >= 3)
						{
					...
					if(($data['0'] != $name[2]) || ($data['11'] != '1'))
					{
						$spl_object->next();
						continue;
					}
					$datas[] = array('trade_no' =>trim($data['3']), 'date' => $checkDate, 'o_status' => 1, 'o_money' => trim($data['7']));
					$spl_object->next();
				}
				$start += $pSize;
