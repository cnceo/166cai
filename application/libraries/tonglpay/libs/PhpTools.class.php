...
//	const  apiUrl = 'http://172.16.1.11:8080/aipg/ProcessServlet';//通联系统对接请求地址（内网）
    ...
    //const apiUrl = 'https://113.108.182.3/aipg/ProcessServlet';//通联系统对接请求地址（外网,商户测试时使用）
        public $apiUrl ;//（生产环境地址，上线时打开该注释）
	
	public function __construct()      
    {      
    	$path = dirname(dirname(__FILE__));
        $this->arrayXml = new ArrayAndXml();
        $ENVFILE = file_get_contents(dirname(__FILE__) . '/../../../../ENV.CON');
        if ($ENVFILE == 'production') {
            $this->password = '111111';
            $this->apiUrl = 'https://tlt.allinpay.com/aipg/ProcessServlet';
            $this->privateKeyFile = $path . '/data/20029000001702704.pem'; //商户私钥证书
            $this->certFile = $path . '/data/allinpay-pds.pem';//通联公钥证书
        } else {
            $this->password = '111111';
            $this->apiUrl = 'https://113.108.182.3/aipg/ProcessServlet';
            $this->privateKeyFile = $path . '/data/20060400000044502.pem'; //商户私钥证书
            $this->certFile = $path . '/data/ceshi/allinpay-pds.pem';//通联公钥证书
        }
    }     
