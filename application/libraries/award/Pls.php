...
class Pls
{
	...
	public function calculate($ctype = '')
	{
		...
		$ainfos = $this->CI->bonus_model->awardInfo(0, 33);
		if(!empty($ainfos))
		{
			foreach ($ainfos as $ainfo)
			{
				...
				$orders = $this->CI->bonus_model->bonusOrders($issue, 33, $this->order_status['draw']);
				$flag = $orders['flag'];
				...
				if(empty($flag))
				{
				    $affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 33, array('key' => 'status', 'val' => $this->order_status['paiqi_ggsucc']));
				    //兼容多次过关计奖状态问题
				    $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 33, array('key' => 'rstatus', 'val' => $this->order_status['paiqi_complete']));
	...
	public function bonus($ctype = '')
	{
		...
		$ainfos = $this->CI->bonus_model->awardInfo(1, 33);
		if(!empty($ainfos))
		{
			foreach ($ainfos as $ainfo)
			{
				$this->CI->bonus_model->trans_start();
				$issue = $this->CI->libcomm->format_issue($ainfo['issue']);
				$orders = $this->CI->bonus_model->bonusOrders($issue, 33, $this->order_status['split_ggwin']);
				$flag = $orders['flag'];
				while(!empty($orders['data']))
				{
					foreach ($orders['data'] as $in => $order)
				...
				if(empty($flag))
				{
				    $affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 33, array('key' => 'rstatus', 'val' => $this->order_status['paiqi_jjsucc']));
				    if($affectedRows)
				    {
				        $returnData['triggerFlag'] = true;
				    }
