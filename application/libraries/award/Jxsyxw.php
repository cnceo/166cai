...
class Jxsyxw
{
	...
	public function calculate($ctype = '')
	{
		...
		$ainfos = $this->CI->bonus_model->awardInfo(0, 21407);
		if(!empty($ainfos))
		{
			foreach ($ainfos as $ainfo)
			{
				$this->CI->bonus_model->trans_start();
				$orders = $this->CI->bonus_model->bonusOrders($ainfo['issue'], 21407, $this->order_status['draw']);
				$flag = $orders['flag'];
				while(!empty($orders['data']))
				{
					...
					$re = $this->CI->bonus_model->setBonusDetail($orders['data'], 21407);
					if(!$re)
					{
				...
				if(empty($flag))
				{
					$affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 21407, array('key' => 'status', 'val' => $this->order_status['paiqi_ggsucc']));
					if($affectedRows)
					{
	...
	public function bonus($ctype = '')
	{
		...
		$ainfos = $this->CI->bonus_model->awardInfo(1, 21407);
		if(!empty($ainfos))
		{
			foreach ($ainfos as $ainfo)
			{
				$this->CI->bonus_model->trans_start();
				$orders = $this->CI->bonus_model->bonusOrders($ainfo['issue'], 21407, $this->order_status['split_ggwin']);
				$flag = $orders['flag'];
				while(!empty($orders['data']))
				{
					foreach ($orders['data'] as $in => $order)
					{
						...
						$orders['data'][$in]['otherBonus'] = $bouns['otherBonus']; //加奖金额
					}
					$re = $this->CI->bonus_model->setBonus($orders['data'], 21407);
					if(!$re)
					{
						$this->CI->bonus_model->trans_rollback();
						return false;
				...
				if(empty($flag))
				{
					$affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 21407, array('key' => 'rstatus', 'val' => $this->order_status['paiqi_jjsucc']));
					if($affectedRows)
					{
						$returnData['triggerFlag'] = true;
					}
	...
	private function cal_bonus($order, $ainfo)
	{
		...
		foreach ($obonus as $bnum)
		{
			...
			$odzjj = (time() > strtotime('2018-03-23 23:30:00')) ? 0 : $this->add_dzjj($order);
			$dzjj += $odzjj;
	 ...
	 * @param array $mbonus	奖金数组
	 * @return array
	 */
	private function activityBonus($order, $mbonus)
	{
		$obonus = json_decode($order['bonus_detail'], true);
		if(in_array($order['playType'], array(3, 5, 7)))
		{
			$moneyArr = array(
					'3' => array('单' => 300, '复' => 400, '胆' => 400),
					'5' => array('单' => 6000, '复' => 8000, '胆' => 8000),
					'7' => array('单' => 300, '复' => 400, '胆' => 400),
			);
			if(strpos($order['codes'], '^') == TRUE)
			{
				$money = $moneyArr[$order['playType']]['单'];
			}
			else if(strpos($order['codes'], '#') == TRUE)
			{
				$money = $moneyArr[$order['playType']]['胆'];
			}
			else
			{
				$money = $moneyArr[$order['playType']]['复'];
			}
			foreach ($obonus as $bnum)
			{
				$mbonus['otherBonus'] += $money * $bnum;
			}
		}
		$mbonus['otherBonus'] = $mbonus['otherBonus'] * $order['multi']; //奖金乘倍数
		$mbonus['bonus'] = $mbonus['bonus'] + $mbonus['otherBonus'];
		$mbonus['margin'] = $mbonus['margin'] + $mbonus['otherBonus'];
