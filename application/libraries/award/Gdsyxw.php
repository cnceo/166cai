...
class Gdsyxw
{
    ...
    public function calculate($ctype = '')
    {
        ...
        $ainfos = $this->CI->bonus_model->awardInfo(0, 21421);
        if(!empty($ainfos))
        {
            foreach ($ainfos as $ainfo)
            {
                $this->CI->bonus_model->trans_start();
                $orders = $this->CI->bonus_model->bonusOrders($ainfo['issue'], 21421, $this->order_status['draw']);
                $flag = $orders['flag'];
                while(!empty($orders['data']))
                {
                    ...
                    $re = $this->CI->bonus_model->setBonusDetail($orders['data'], 21421);
                    if(!$re)
                    {
                ...
                if(empty($flag))
                {
                    $affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 21421, array('key' => 'status', 'val' => $this->order_status['paiqi_ggsucc']));
                    if($affectedRows)
                    {
    ...
    public function bonus($ctype = '')
    {
        ...
        $ainfos = $this->CI->bonus_model->awardInfo(1, 21421);
        if(!empty($ainfos))
        {
            foreach ($ainfos as $ainfo)
            {
                $this->CI->bonus_model->trans_start();
                $orders = $this->CI->bonus_model->bonusOrders($ainfo['issue'], 21421, $this->order_status['split_ggwin']);
                $flag = $orders['flag'];
                while(!empty($orders['data']))
                {
                    foreach ($orders['data'] as $in => $order)
                    ...
                    $re = $this->CI->bonus_model->setBonus($orders['data'], 21421);
                    if(!$re)
                    {
                        $this->CI->bonus_model->trans_rollback();
                        return false;
                ...
                if(empty($flag))
                {
                    $affectedRows = $this->CI->bonus_model->setPaiqiStatus($ainfo['issue'], 21421, array('key' => 'rstatus', 'val' => $this->order_status['paiqi_jjsucc']));
                    if($affectedRows)
                    {
                        $returnData['triggerFlag'] = true;
                    }
    ...
    private function cal_bonus($order, $ainfo)
    {
        ...
        foreach ($obonus as $bnum)
        {
            ...
            $odzjj = 0; //$this->add_dzjj($order); 删除加奖（2018/04/15）
            $dzjj += $odzjj;
