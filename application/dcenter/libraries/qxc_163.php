...
class Qxc_163
{
    ...
    public function capture($param,$data)
    {
        ...
        if(!empty($data['issues']))
        {
            ...
            {
                $url = 'http://caipiao.163.com/award/qxc/'.$issue.'.html';
    ...
    private function get_datas($url, $param)
    {
        ...
        //开奖球号
        $rulesBall = '/<p.*?zj_area.*?>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<\/p>/i';
