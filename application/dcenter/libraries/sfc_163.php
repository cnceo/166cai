...
class Sfc_163
{
    ...
    private function get_datas($param, $issue, $mname)
    {
        ...
        $url = 'http://live.caipiao.163.com/zcbf/?date='.$issue.'&gameType=sfc';
        ...
        unset($matches[0]);
        $this->updateSfcScore($matches, $param['source'], $issue, $mname);
    }
    ...
    private function updateSfcScore($matches, $source, $issue, $mname)
    {
        if(!empty($matches))
        {
            $fields = array('mid', 'mname', 'half_score', 'full_score', 'result', 'status', 'source', 'created');
            $bdata['s_data'] = array();
            $bdata['d_data'] = array();
            $count = 0;
            foreach ($matches[1] as $key => $match)
            {
                ...
                array_push($bdata['d_data'], $this->getResult($matches[3][$key]));
     ...
     * @date:2015-03-12
     */
    private function getResult($status)
    {
        $state = array(
            '胜' => '3',
            '平' => '1',
