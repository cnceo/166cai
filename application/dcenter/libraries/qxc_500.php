...
class Qxc_500
{
    ...
    public function capture($param,$data)
    {
        ...
        if(!empty($data['issues']))
        {
            ...
            {
                $url = 'http://kaijiang.500.com/shtml/qxc/'.$issue.'.shtml';
    ...
    private function get_datas($url, $param)
    {
        ...
        $rulesBall = '<div.*?ball_box01.*?>.*?<ul.*?>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<li.*?ball_orange.*?>(\d+)<\/li>.*?<\/ul>.*?<\/div>';
        ...
        //获取销量，滚存
        $rulesPool = '<td>.*?<span.*?cfont1.*?>(.*?)元<\/span>.*?<span.*?cfont1.*?>(.*?)元<\/span>.*?<\/td>';
