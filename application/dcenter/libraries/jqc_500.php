...
class Jqc_500
{
    ...
    {
    	$url = 'http://live.500.com/4chang.php?e=' . $issue;
    	...
    	unset($content[0]);
    	$this->updateJqcScore($content, $param['source'], $issue, $mname);
    }
	...
	private function updateJqcScore($matches, $source, $issue, $mname)
    {
    	if(!empty($matches))
    	{
    		...
    		$fields = array('mid', 'mname', 'league', 'home', 'away', 'half_score', 'full_score', 'home_result', 'away_result', 'status', 'source', 'created');
    		$bdata['s_data'] = array();
    		$bdata['d_data'] = array();
    		$count = 0;
