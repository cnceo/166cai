...
class Fc3d_163
{
    ...
    public function capture($param,$data)
    {
        ...
        if(!empty($data['issues']))
        {
            ...
            {
                $url = 'http://caipiao.163.com/award/3d/'.$issue.'.html';
        ...
        // if(empty($issue))
        // {
        //     $url = 'http://caipiao.163.com/award/3d/';
        // }else{
        //     $url = 'http://caipiao.163.com/award/3d/'.$issue.'.html';
        // }
    ...
    private function get_datas($url, $param)
    {
        ...
        //开奖球号
        $rulesBall = '/<p.*?zj_area.*?>[^<]+<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<\/p>/i';
        ...
        $awardDetail['sale'] = '0'; //$this->CI->lib_comm->format_num($details[2]);
        $awardDetail['pool'] = '0'; //$this->CI->lib_comm->format_num($details[3]);
