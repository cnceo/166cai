...
class Pl5_163
{
    ...
    public function capture($param,$data)
    {
        ...
        if(!empty($data['issues']))
        {
            ...
            {
                $url = 'http://caipiao.163.com/award/pl5/'.$issue.'.html';
        ...
        // if(empty($issue))
        // {
        //     $url = 'http://caipiao.163.com/award/pl5/';
        // }else{
        //     $url = 'http://caipiao.163.com/award/pl5/'.$issue.'.html';
        // }
    ...
    private function get_datas($url, $param)
    {
        ...
        //开奖球号
        $rulesBall = '/<p.*?zj_area.*?>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<span.*?red_ball.*?>(\d+)<\/span>[\s\S]*?<\/p>/i';
        ...
        $rulePools2 = '/<tr.*?>.*?<td.*?>.*?<\/td>.*?<td.*?>.*?<span.*?award1.*?>(.*?)<\/span>.*?<\/td>.*?<td.*?awardvalue1.*?>(.*?)<\/td>.*?<\/tr>/is';
        preg_match($rulePools2,$pools[1],$pool);

        $dfields = array();
        $dfields['zx']['zs'] = $this->CI->lib_comm->format_num($pool[1]);
        $dfields['zx']['dzjj'] = $this->CI->lib_comm->format_num($pool[2]);
