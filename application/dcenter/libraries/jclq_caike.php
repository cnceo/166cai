...
class Jclq_caike
{
    ...
    private function get_datas($date, $param, $mids)
    {
    	...
    	$post_data['txtStartDate'] = date('Y-m-d', strtotime('-1 day', strtotime($date)));
    	$post_data['Button1'] = '';
        ...
        unset($matches[0]);
        $this->updateJclqScore($matches, $param['source'], $date_arr, $mids);
    }
     ...
     * @date:2015-03-18
     */
    private function updateJclqScore($datas, $source, $date_arr, $mids)
    {
    	...
    	if(!empty($datas))
    	{
    		...
    		foreach ($datas[1] as $in => $val)
    		{
    			...
    			array_push($bdata['d_data'], $datas[8][$in]);
    			$score = trim($datas[7][$in]) . ':' . trim($datas[6][$in]);
    			$full_score = $this->CI->lib_comm->score_filter($score);
