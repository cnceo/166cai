...
class Result_Bqc_500
{
    ...
    public function capture($param, $data)
    {
    	...
    	foreach ($issues as $issue)
    	{
    		...
    		$url = "http://kaijiang.500.com/shtml/zc6/{$issue}.shtml";
    ...
    private function get_datas($source, $content)
    {
    	...
    	$reg .= '<table.*?kj_tablelist02.*?>.*?<tr.*?<\/tr>.*?<tr.*?<\/tr>.*?<tr.*?<\/tr>.*?';
		...
		if(empty($d_data['mid'])) return ; //防止mid为空时插入数据库
		foreach ($results[1] as $key=>$val)
		{
			$val = trim($val);
			if(!in_array($val, array('3','1','0','*')))
			{
				$results[1][$key] = '*';
			}
		}
		...
		$d_data['award'] = '0'; //preg_replace('/[^\d\.]+/is', '', $matches[5]);
		...
		$this->updateBqcScore($s_data, $d_data);
		return $issues[1];
    }
