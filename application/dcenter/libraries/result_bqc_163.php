...
class Result_bqc_163
{
    ...
    {
    	$url = 'http://caipiao.163.com/award/bqc/'.$issue.'.html';
        $content = $this->CI->tools->get_content($url, __CLASS__, 0, array('ENCODING' => 'gzip'));

        //获取当前最新期号
        $ruleDetail = '<option.*?value="'.$issue.'".*?sale="(.*?)".*?matchball="(.*?)".*?bonus1=".*?,(.*?)".*?selected\s*>(.*?)<\/option>';
        ...
        if(!empty($detail))
        {
            ...
            $awardDetail['status'] = $this->CI->lib_comm->getStatus($awardDetail['result'], array('lid' => 'tczq', 'issue' => $awardDetail['mid'], 'ctype' => 2));
			$awardDetail['rstatus'] = $awardDetail['sale'] > 0 ? 1 : 0;
			
            $s_data = array();
            array_push($s_data, "(?, ?, ?, ?, ?, ?, ?, {$param['source']}, now())");
            $this->updateBqcScore($s_data, $awardDetail);
        }

    }

    private function updateBqcScore($s_data, $d_data)
    {
        $bdata['s_data'] = $s_data;
        $bdata['d_data'] = $d_data;
        $this->CI->data_model->insertBqcResult($bdata);
    }
