...
class Result_jqc_caike
{
    ...
    private function get_datas($issue, $param)
    {
    	...
    	$this->updateJqcResult($respone, $param['source']);
    }
     ...
     * @date:2015-03-19
     */
    private function updateJqcResult($datas, $source)
    {
    	...
    	if(!empty($datas))
    	{
    		...
    		$sale = $this->CI->lib_comm->format_num($matchs[1]);
    		$award = '0';
    		unset($matchs);
    		...
    		$bdata['s_data'] = $s_data;
    		$bdata['d_data'] = $d_data;
    		$res = $this->CI->data_model->insertJqcResult($bdata);
    		if(!$res)
    		{
