...
class Sfc_caike
{
    ...
    {
    	$url = "http://www.310win.com/Info/Result/Soccer.aspx?load=ajax&typeID=1&IssueID={$issue}&randomT-_-=" . time();
    	$respone = $this->CI->tools->get_content($url, __CLASS__);
    	$respone = json_decode($respone, true);
    	$this->updateSfcScore($respone, $param['source'], $mname);
    }
     ...
     * @date:2015-03-19
     */
    private function updateSfcScore($datas, $source, $mname)
    {

    	if(!empty($datas['Table']))
    	{
    		$fields = array('mid', 'mname', 'league', 'home', 'away', 'half_score', 'full_score', 'status', 'result', 'source', 'created');
    		$bdata['s_data'] = array();
    		$bdata['d_data'] = array();
    		$count = 0;
    		foreach ($datas['Table'] as $val)
    		{
    			...
    			array_push($bdata['d_data'], $val['Result']);
