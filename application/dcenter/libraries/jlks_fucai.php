...
class Jlks_Fucai
{
	...
	{
		$issues = $this->CI->gaopin_model->getLotteryIssues('cp_jlks_paiqi');
		if($issues)
		{
	...
	public function saveData($issues, $awardArr)
	{
		...
		foreach ($issues as $issue)
		{
			if(isset($awardArr[$issue]))
			{
				...
				$result = $this->CI->gaopin_model->updateByIssue('cp_jlks_paiqi', $issue, $data);
				if($result)
				{
					$synflag = true;
				}
		...
		{
			//启动同步号码任务
			$this->CI->gaopin_model->updateTicketStop(1, '56', 0);
		}
	}
}
