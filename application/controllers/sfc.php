...
class Sfc extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $view = $data['view'];
        unset($data['view']);
        $this->display('sfc/'. $view,$data, 'v1.1');
    }
    /**
     * [dssc 单式上传]
    ...
    {
        $this->display('sfc/dssc',$this->getData(1), 'v1.1'); 
    }
    /**
     * [getData 获取数据]
     * @author LiKangJian 2017-08-12
    ...
    private function getData($type = 0)
    {
        ...
        if (empty($issueId)) {
            $endissue = end($issues);
            $this->redirect('/sfc?issue='.$endissue['seExpect']);
        }
        ...
        foreach ($issues as $key => $issue)
        {
            ...
            if ($issue['seExpect'] == $minCurrentId)
            {
        ...
        if ( ! empty($apiUrl))
        {
            $content = $this->dataSource->readEuropeOdds(SFC, $issueId, 0);
