<?php
class Syxw extends MY_Controller
{
	public static $TYPE_MAP = array (
			...
			'lexuan5' => array (
					...
					'bonus' => '19-1384元',
					'rule' => array (
							'玩法说明：选择3个号码，与开奖号任意3个相同即中奖19元，与开奖号的前三位号码相同（顺序不限）即中奖214元，与开奖号的前三位号码相同且顺序一致即中奖1384元！',
							'玩法说明：选择4个号码，与开奖号码任意4个相同即中奖154元，与开奖号码任意3个相同即中奖19元！',
							'玩法说明：选择5个号码，与开奖号码任意5个相同即中奖1080元，与开奖号码任意4个相同即中奖90元！'
					)
			)
	);
	...
	public function index()
	{
		...
		$awardArr = unserialize($this->cache->get ( $REDIS ['SYXW_AWARD'] ));
		$miss = unserialize($this->cache->get ( $REDIS ['SYXW_MISS'] ));
		$info = json_decode ( $this->cache->get ( $REDIS ['SYXW_ISSUE_TZ'] ), true );
		$lastIssue = empty ( $info ['aIssue'] ) ? $info ['lIssue'] ['seExpect'] : $info ['aIssue'] ['seExpect'];
		$followIssues = json_decode ( $this->cache->hGet ( $REDIS ['ISSUE_COMING'], 'SYXW' ), true );
		...
		$odatas = array ();
		if ($this->uid) $odatas = $this->order_model->getNewOrders ( $this->uid, SYXW, 0, 5);
		...
		$this->display ( 'syxw/index', $data, 'v1.2' );
	}
