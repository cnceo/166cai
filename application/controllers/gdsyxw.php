<?php
class Gdsyxw extends MY_Controller
{
	...
	{
		// 初始化
		$lotteryId = Lottery_Model::GDSYXW;
		...
		$awardArr = unserialize($this->cache->get ( $REDIS ['GDSYXW_AWARD'] ));
		$miss = unserialize($this->cache->get ( $REDIS ['GDSYXW_MISS'] ));
		$info = json_decode ( $this->cache->get ( $REDIS ['GDSYXW_ISSUE_TZ'] ), true );
		$lastIssue = empty ( $info ['aIssue'] ) ? $info ['lIssue'] ['seExpect'] : $info ['aIssue'] ['seExpect'];
		$followIssues = json_decode ( $this->cache->hGet ( $REDIS ['ISSUE_COMING'], 'GDSYXW' ), true );
		// 生成追号期次
		...
		$odatas = array ();
		if ($this->uid) $odatas = $this->order_model->getNewOrders ( $this->uid, GDSYXW, 0, 5);
		...
		$this->display ( 'gdsyxw/index', $data, 'v1.2' );
	}
