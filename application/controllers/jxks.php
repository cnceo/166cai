<?php
class Jxks extends MY_Controller {
	...
	public function index()
	{
		...
		$info = json_decode ( $this->cache->get ( $REDIS ['JXKS_ISSUE_TZ'] ), true );
		$miss = json_decode ($this->cache->get ( $REDIS ['JXKS_MISS'] ), true );
		$lastIssue = empty ( $info ['aIssue'] ) ? $info ['lIssue'] ['seExpect'] : $info ['aIssue'] ['seExpect'];
		$awardArr = unserialize($this->cache->get ( $REDIS ['JXKS_AWARD'] ));
		$followIssues = json_decode ( $this->cache->hGet ( $REDIS ['ISSUE_COMING'], 'JXKS' ), true );
		...
		$rest = 85-substr($info ['cIssue'] ['seExpect'], -2);
		...
		$this->load->view('v1.2/jxks/index', $data);
	}
