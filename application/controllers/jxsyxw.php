<?php
class Jxsyxw extends MY_Controller
{
	...
	{
		// 初始化
		$lotteryId = Lottery_Model::JXSYXW;
		...
		$awardArr = unserialize($this->cache->get ( $REDIS ['JXSYXW_AWARD'] ));
		$miss = unserialize($this->cache->get ( $REDIS ['JXSYXW_MISS'] ));
		$info = json_decode ( $this->cache->get ( $REDIS ['JXSYXW_ISSUE_TZ'] ), true );
		$lastIssue = empty ( $info ['aIssue'] ) ? $info ['lIssue'] ['seExpect'] : $info ['aIssue'] ['seExpect'];
		$followIssues = json_decode ( $this->cache->hGet ( $REDIS ['ISSUE_COMING'], 'JXSYXW' ), true );
		// 生成追号期次
		$multi = !empty( $inputGetArr['multi'] ) ? $inputGetArr['multi'] : 1;
		$chases = array();
		$issue = 1;
		while (count($chases) < 84 && $issue) {
		...
		$odatas = array ();
		if ($this->uid) $odatas = $this->order_model->getNewOrders ( $this->uid, JXSYXW, 0, 5);
		...
		$this->display ( 'jxsyxw/index', $data, 'v1.2' );
	}
